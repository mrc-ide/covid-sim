	...
	if (fst)
	{
		...
		for (int i = 0; i < P.PopSize; i++)
		{
			...
			if ((x >= 0) && (x < P.b.width) && (y >= 0) && (y < P.b.height))
			{
				...
				{
					bmPopulation[j]++;
					if (bmPopulation[j] > maxPop)
						maxPop = bmPopulation[j];
				}
			}
		}
				...
				if ((i < P.NMC - 1) &&
						...
						((Mcells[i].country != Mcells[i + 1].country) ||
						 ((P.DoAdunitBoundaryOutput) &&
							((AdUnits[Mcells[i].adunit].id % P.AdunitLevel1Mask) / P.AdunitBitmapDivisor !=
							 (AdUnits[Mcells[i + 1].adunit].id % P.AdunitLevel1Mask) / P.AdunitBitmapDivisor))))
					f = 1;
				if ((i > 0) &&
						(i / P.get_number_of_micro_cells_high() ==
						 (i - 1) / P.get_number_of_micro_cells_high()) &&
						(Mcells[i - 1].n > 0) && (Mcells[i].country != Mcells[i - 1].country))
					f = 1;
				if ((i < P.NMC - P.get_number_of_micro_cells_high()) &&
						(Mcells[i + P.get_number_of_micro_cells_high()].n > 0) &&
						((Mcells[i].country != Mcells[i + P.get_number_of_micro_cells_high()].country) ||
						 ((P.DoAdunitBoundaryOutput) &&
							((AdUnits[Mcells[i].adunit].id % P.AdunitLevel1Mask) / P.AdunitBitmapDivisor !=
							 (AdUnits[Mcells[i + P.get_number_of_micro_cells_high()].adunit].id %
								P.AdunitLevel1Mask) /
									 P.AdunitBitmapDivisor))))
					f = 1;
				if ((i >= P.get_number_of_micro_cells_high()) &&
						(Mcells[i - P.get_number_of_micro_cells_high()].n > 0) &&
						(Mcells[i].country != Mcells[i - P.get_number_of_micro_cells_high()].country))
					f = 1;
				if (f)
				{
					x = (int)(P.in_microcells_.width *
										(((double)(i / P.get_number_of_micro_cells_high())) + 0.5) * P.scale.x) -
							P.bmin.x;
					y = (int)(P.in_microcells_.height *
										(((double)(i % P.get_number_of_micro_cells_high())) + 0.5) * P.scale.y) -
							P.bmin.y;
					if ((x >= 0) && (x < P.b.width) && (y >= 0) && (y < P.b.height))
					{
						j = y * bmh->width + x;
...
#pragma omp parallel for schedule(static, 5000) default(none)                                      \
	...
	for (int i = 0; i < mi; i++)
	{
		if (bmPopulation[i] == -1)
			...
			bmPixels[i] = (unsigned char)(BWCOLS + BWCOLS * log((double)bmInfected[i]) /
																								 logMaxPop); /* red for infected */
		else if (bmTreated[i] > 0)
			bmPixels[i] = (unsigned char)(2 * BWCOLS + BWCOLS * log((double)bmTreated[i]) /
																										 logMaxPop); /* blue for treated */
		else if (bmRecovered[i] > 0)
			bmPixels[i] = (unsigned char)(3 * BWCOLS + BWCOLS * log((double)bmRecovered[i]) /
																										 logMaxPop); /* green for recovered */
		else if (bmPopulation[i] > 0)
			bmPixels[i] = (unsigned char)(BWCOLS * log((double)bmPopulation[i]) /
																		logMaxPop); /* grey for just people */
		else
			bmPixels[i] = 0;
	}
}
...
void OutputBitmap(int tp)
{
	...
	if (P.BitmapFormat == BitmapFormats::PNG)
	{
		...
		if (!(dat = fopen(buf, "wb")))
			ERR_CRITICAL("Unable to open bitmap file\n");
		fprintf(dat, "BM");
		// fwrite_big((void *) &bmf,sizeof(unsigned char),(sizeof(bitmap_header)/sizeof(unsigned
		// char))+bmh->imagesize,dat);
		fwrite_big((void *)bmf, sizeof(bitmap_header), 1, dat);
		for (int i = 0; i < bmh->imagesize; i++)
			fputc(bmPixels[i], dat);
		fclose(dat);
		Image bmap(buf);
		...
		bmap.write(buf);
#elif defined(_WIN32)
		// Windows specific bitmap manipulation code - could be recoded using LIBGD or another unix
		// graphics library
		...
		// if ((P.OutputBitmap == 1) && (tp == 0)) AddAviFrame(avi, bmpdib, (unsigned
		// char*)(&bmh->palette[0][0]));
		...
		if ((cn1 == 1) && (tp == 0))
		{
			...
			if (!palette)
				ERR_CRITICAL("Unable to allocate palette memory\n");
		...
		// Now save as png
		sprintf(buf, "%s.%05i.png", OutF,
						j + 1); // sprintf(buf,"%s.ge" DIRECTORY_SEPARATOR "%s.%05i.png",OutFileBase,OutF,j+1);
	...
	else if (P.BitmapFormat == BitmapFormats::BMP)
	{
		...
		fprintf(dat, "BM");
		fwrite_big((void *)bmf, sizeof(unsigned char),
							 sizeof(BitmapHeader) / sizeof(unsigned char) + bmh->imagesize, dat);
		fclose(dat);
	}
	else
	{
...
void InitBMHead()
{
	...
	{
#ifdef _WIN32
		bmpdib = CreateDIBSection(GetDC(NULL), (BITMAPINFO *)bmp, DIB_RGB_COLORS, (void **)&bmPixels,
															NULL, NULL);
		...
		{
			// Visual Studio Analyze incorrectly reports this because it doesn't understand
			// Gdiplus::GetImageEncodersSize() warning C6385: Reading invalid data from 'pImageCodecInfo':
			// the readable size is 'size' bytes, but '208' bytes may be read.
