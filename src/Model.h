...
struct PopVar
{
	int S, L, I, R, D, cumI, cumR, cumD, cumC, cumTC, cumFC, cumDC, trigDetectedCases, cumTG, cumSI,
			nTG;
	int cumH;												 // Added cumulative hospitalisation: ggilani 28/10/14
	int cumCT, cumCC, DCT, cumDCT;	 // Added total and cumulative contact tracing: ggilani 15/06/17,
																	 // and equivalents for digital contact tracing: ggilani 11/03/20
	...
	int cumI_adunit[MAX_ADUNITS], cumC_adunit[MAX_ADUNITS], cumD_adunit[MAX_ADUNITS],
			...
			cumDC_adunit[MAX_ADUNITS]; // added cumulative hospitalisation per admin unit: ggilani
																 // 28/10/14, cumulative detected cases per adunit: ggilani 03/02/15
	int cumCT_adunit[MAX_ADUNITS], cumCC_adunit[MAX_ADUNITS],
			trigDC_adunit[MAX_ADUNITS]; // added cumulative CT per admin unit: ggilani 15/06/17
	int cumDCT_adunit[MAX_ADUNITS],
			DCT_adunit[MAX_ADUNITS]; // added cumulative and overall digital contact tracing per adunit:
															 // ggilani 11/03/20
	int cumItype[INFECT_TYPE_MASK], cumI_keyworker[2], cumC_keyworker[2], cumT_keyworker[2];
	Infection *inf_queue[MAX_NUM_THREADS]; // the queue (i.e. list) of infections. 1st index is
																				 // thread, 2nd is person.
	int n_queue[MAX_NUM_THREADS];					 // number of infections in inf_queue
	int *p_queue[NUM_PLACE_TYPES], *pg_queue[NUM_PLACE_TYPES],
			np_queue[NUM_PLACE_TYPES]; // np_queue is number of places in place queue (by place type),
																 // p_queue, and pg_queue is the actual place and place-group queue
																 // (i.e. list) of places. 1st index is place type, 2nd is place.
	...
	double sumRad2, maxRad2, cumT, cumV, cumVG, cumUT, cumTP, cumV_daily,
			cumVG_daily; // added cumVG, cumVG_daily
	...
	int Mild, ILI, SARI, Critical, CritRecov, /*cumulative incidence*/ cumMild, cumILI, cumSARI,
			cumCritical, cumCritRecov;
	int Mild_adunit[MAX_ADUNITS], ILI_adunit[MAX_ADUNITS], SARI_adunit[MAX_ADUNITS],
			Critical_adunit[MAX_ADUNITS], CritRecov_adunit[MAX_ADUNITS];
	/// cum incidence quantities. (+ by admin unit)
	int cumMild_adunit[MAX_ADUNITS], cumILI_adunit[MAX_ADUNITS], cumSARI_adunit[MAX_ADUNITS],
			cumCritical_adunit[MAX_ADUNITS], cumCritRecov_adunit[MAX_ADUNITS];
	int Mild_age[NUM_AGE_GROUPS], ILI_age[NUM_AGE_GROUPS], SARI_age[NUM_AGE_GROUPS],
			Critical_age[NUM_AGE_GROUPS], CritRecov_age[NUM_AGE_GROUPS];
	/// cum incidence quantities. (+ by age group)
	int cumMild_age[NUM_AGE_GROUPS], cumILI_age[NUM_AGE_GROUPS], cumSARI_age[NUM_AGE_GROUPS],
			cumCritical_age[NUM_AGE_GROUPS], cumCritRecov_age[NUM_AGE_GROUPS];

	int cumDeath_ILI, cumDeath_SARI,
			cumDeath_Critical; // tracks cumulative deaths from ILI, SARI & Critical severities
	int cumDeath_ILI_adunit[MAX_ADUNITS], cumDeath_SARI_adunit[MAX_ADUNITS],
			cumDeath_Critical_adunit[MAX_ADUNITS]; // tracks cumulative deaths from ILI, SARI & Critical
																						 // severities
	int cumDeath_ILI_age[NUM_AGE_GROUPS], cumDeath_SARI_age[NUM_AGE_GROUPS],
			cumDeath_Critical_age[NUM_AGE_GROUPS];

	int **prevInf_age_adunit, **cumInf_age_adunit; // prevalence, incidence, and cumulative incidence
																								 // of infection by age and admin unit.
...
struct Results
{
	...
	double t;
	double **prevInf_age_adunit, **incInf_age_adunit,
			**cumInf_age_adunit; // prevalence, incidence, and cumulative incidence of infection by age
													 // and admin unit.
	...
	double CT, incCT, incCC, DCT,
			incDCT; // added total numbers being contact traced and incidence of contact tracing: ggilani
							// 15/06/17, and for digital contact tracing: ggilani 11/03/20
	...
	double incI_adunit[MAX_ADUNITS], incC_adunit[MAX_ADUNITS], cumT_adunit[MAX_ADUNITS],
			incD_adunit[MAX_ADUNITS], cumD_adunit[MAX_ADUNITS], incH_adunit[MAX_ADUNITS],
			incDC_adunit[MAX_ADUNITS]; // added incidence of hospitalisation per day: ggilani 28/10/14,
																 // incidence of detected cases per adunit,: ggilani 03/02/15
	double incCT_adunit[MAX_ADUNITS], incCC_adunit[MAX_ADUNITS], incDCT_adunit[MAX_ADUNITS],
			DCT_adunit[MAX_ADUNITS]; // added incidence of contact tracing and number of people being
															 // contact traced per admin unit: ggilani 15/06/17
	double incI_keyworker[2], incC_keyworker[2], cumT_keyworker[2];

	///@{
	/** Severity States track the COVID-19 states (e.g., mild, critical, etc.) */
	double Mild, ILI, SARI, Critical,
			CritRecov; // Prevalence				//// Must be: i) initialised to zero in SetUpModel. ii)
								 // outputted in SaveResults iii) outputted in SaveSummaryResults
	double incMild, incILI, incSARI, incCritical,
			incCritRecov; // Incidence				//// Must be: i) initialised to zero in SetUpModel. ii)
										// calculated in RecordSample iii) outputted in SaveResults.
	double cumMild, cumILI, cumSARI, cumCritical,
			cumCritRecov; // cumulative incidence		//// Must be: i) initialised to zero in SetUpModel.
										// ii) outputted in SaveResults
	double incDeath_ILI, incDeath_SARI,
			incDeath_Critical; // tracks incidence of death from ILI, SARI & Critical severities
	double cumDeath_ILI, cumDeath_SARI,
			cumDeath_Critical; // tracks cumulative deaths from ILI, SARI & Critical severities
	///@}

	/////// Severity States by admin unit
	double Mild_adunit[MAX_ADUNITS], ILI_adunit[MAX_ADUNITS], SARI_adunit[MAX_ADUNITS],
			Critical_adunit[MAX_ADUNITS], CritRecov_adunit[MAX_ADUNITS]; // Prevalence by admin unit
	double incMild_adunit[MAX_ADUNITS], incILI_adunit[MAX_ADUNITS], incSARI_adunit[MAX_ADUNITS],
			incCritical_adunit[MAX_ADUNITS], incCritRecov_adunit[MAX_ADUNITS]; // incidence by admin unit
	double cumMild_adunit[MAX_ADUNITS], cumILI_adunit[MAX_ADUNITS], cumSARI_adunit[MAX_ADUNITS],
			cumCritical_adunit[MAX_ADUNITS],
			cumCritRecov_adunit[MAX_ADUNITS]; // cumulative incidence by admin unit
	double incDeath_ILI_adunit[MAX_ADUNITS], incDeath_SARI_adunit[MAX_ADUNITS],
			incDeath_Critical_adunit[MAX_ADUNITS]; // tracks incidence of death from ILI, SARI & Critical
																						 // severities
	double cumDeath_ILI_adunit[MAX_ADUNITS], cumDeath_SARI_adunit[MAX_ADUNITS],
			cumDeath_Critical_adunit[MAX_ADUNITS]; // tracks cumulative deaths from ILI, SARI & Critical
																						 // severities

	/////// Severity States by age group
	double Mild_age[NUM_AGE_GROUPS], ILI_age[NUM_AGE_GROUPS], SARI_age[NUM_AGE_GROUPS],
			Critical_age[NUM_AGE_GROUPS], CritRecov_age[NUM_AGE_GROUPS]; // Prevalence by admin unit
	double incMild_age[NUM_AGE_GROUPS], incILI_age[NUM_AGE_GROUPS], incSARI_age[NUM_AGE_GROUPS],
			incCritical_age[NUM_AGE_GROUPS], incCritRecov_age[NUM_AGE_GROUPS]; // incidence by admin unit
	double cumMild_age[NUM_AGE_GROUPS], cumILI_age[NUM_AGE_GROUPS], cumSARI_age[NUM_AGE_GROUPS],
			cumCritical_age[NUM_AGE_GROUPS],
			cumCritRecov_age[NUM_AGE_GROUPS]; // cumulative incidence by admin unit
	double incDeath_ILI_age[NUM_AGE_GROUPS], incDeath_SARI_age[NUM_AGE_GROUPS],
			incDeath_Critical_age[NUM_AGE_GROUPS]; // tracks incidence of death from ILI, SARI & Critical
																						 // severities
	double cumDeath_ILI_age[NUM_AGE_GROUPS], cumDeath_SARI_age[NUM_AGE_GROUPS],
			cumDeath_Critical_age[NUM_AGE_GROUPS]; // tracks cumulative deaths from ILI, SARI & Critical
																						 // severities

	double prevQuarNotInfected, prevQuarNotSymptomatic; // Which people are under quarantine but not
																											// themselves infected/sypmtomatic?

	/////// possibly need quantities by age (later)
	//// state variables (S, L, I, R) and therefore (Mild, ILI) etc. changed in i) SetUpModel
	///(initialised to zero); ii)
...
struct Events
{
	...
	int run, infectee_ind, infector_ind, type, infectee_adunit, listpos, infectee_cell, infector_cell,
			thread;
};
...
/*
	...
	AA - Absent working adults who are caring for sick children (only assigned if no non-working,
	quarantine or sick adults available). ACS - Children below care requirement cut-off age who are
	absent (sick or quarantined)

	APC - Non-quarantined working adult cases absent due to closure of their workplace (excl
	teachers). APA - Absent working adults who are caring for children at home due to school closure
	(only assigned if no non-working, quarantine or sick adults available). ACS - Children below care
	requirement cut-off age who are absent due to school closure
...
struct Intervention
{
	...
	double StartThresholdHigh, StartThresholdLow, StopThreshold, Level, LevelCellVar, LevelAUVar,
			LevelCountryVar, ControlParam, LevelClustering;
...
struct AdminUnit
{
	...
	double SocialDistanceTimeStart,
			PlaceCloseTimeStart; // added these to admin unit in the hope of getting specific start times
													 // for Italy: ggilani 16/03/20
	...
	double SocialDistanceDuration, HQuarantineDuration, CaseIsolationPolicyDuration,
			PlaceCloseDuration, DCTDuration;
	int *dct, ndct;			 // arrays for admin unit based digital contact tracing: ggilani 10/03/20
	double *origin_dest; // storage for origin-destination matrix between admin units: ggilani
											 // 28/01/15
};
...
extern double inftype[INFECT_TYPE_MASK], inftype_av[INFECT_TYPE_MASK], infcountry[MAX_COUNTRIES],
...
extern double inf_household[MAX_HOUSEHOLD_SIZE + 1][MAX_HOUSEHOLD_SIZE + 1],
		denom_household[MAX_HOUSEHOLD_SIZE + 1];
extern double inf_household_av[MAX_HOUSEHOLD_SIZE + 1][MAX_HOUSEHOLD_SIZE + 1],
		AgeDist[NUM_AGE_GROUPS], AgeDist2[NUM_AGE_GROUPS];
extern double case_household[MAX_HOUSEHOLD_SIZE + 1][MAX_HOUSEHOLD_SIZE + 1],
...
extern double PropPlacesC[NUM_AGE_GROUPS * AGE_GROUP_WIDTH][NUM_PLACE_TYPES],
		AirTravelDist[MAX_DIST];
