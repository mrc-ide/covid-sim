...
void SaveSummaryResults(void) //// calculates and saves summary results (called for average of extinct and non-extinct realisation time series - look in main) 
{
	...
	if (P.DoSeverity)
	{
		...
		for (i = 0; i < P.NumSamples; i++)
		{
			if (i > 0)
			{
				SARI = (TimeSeries[i].SARI - TimeSeries[i - 1].SARI) * sc2 + SARI * sc1;
				Critical = (TimeSeries[i].Critical - TimeSeries[i - 1].Critical) * sc2 + Critical * sc1;
				CritRecov = (TimeSeries[i].CritRecov - TimeSeries[i - 1].CritRecov) * sc2 + CritRecov * sc1;
				incSARI = TimeSeries[i].incSARI * (1.0 - sc2) + incSARI * sc1;
				incCritical = TimeSeries[i].incCritical * (1.0 - sc2) + incCritical * sc1;
				incCritRecov = TimeSeries[i].incCritRecov * (1.0 - sc2) + incCritRecov * sc1;
			}
			else
			{
				SARI = TimeSeries[i].SARI * sc2;
				Critical = TimeSeries[i].Critical * sc2;
				CritRecov = TimeSeries[i].CritRecov * sc2;
			}

			fprintf(dat, "%.10f\t%.10f\t%.10f\t%.10f\t%.10f\t%.10f\t%.10f\t%.10f\t%.10f\t%.10f\t%.10f\t%.10f\t%.10f\t%.10f\t%.10f\t%.10f\t%.10f\t%.10f\t%.10f\t%.10f\t%.10f\t%.10f\n",
				TimeSeries[i].t, TimeSeries[i].S, TimeSeries[i].I, TimeSeries[i].R, TimeSeries[i].incI,
				TimeSeries[i].Mild, TimeSeries[i].ILI, TimeSeries[i].SARI - SARI, TimeSeries[i].Critical - Critical, TimeSeries[i].CritRecov - CritRecov,
				TimeSeries[i].incMild, TimeSeries[i].incILI, incSARI, incCritical, incCritRecov, TimeSeries[i].incD,
				TimeSeries[i].cumMild, TimeSeries[i].cumILI, TimeSeries[i].cumSARI, TimeSeries[i].cumCritical, TimeSeries[i].cumCritRecov, TimeSeries[i].D);
		}
		fclose(dat);
		...
		if ((P.DoAdUnits) && (P.OutputSeverityAdminUnit))
		{
			...
			for (i = 0; i < P.NumSamples; i++)
			{
				...
				//// prevalance
				for (j = 0; j < P.NumAdunits; j++)		fprintf(dat, "\t%.10f", TimeSeries[i].Mild_adunit[j] + 0.0);
				for (j = 0; j < P.NumAdunits; j++)		fprintf(dat, "\t%.10f", TimeSeries[i].ILI_adunit[j] + 0.0);
				for (j = 0; j < P.NumAdunits; j++)		fprintf(dat, "\t%.10f", (TimeSeries[i].SARI_adunit[j] - SARI_a[j]) + 0.0);
				for (j = 0; j < P.NumAdunits; j++)		fprintf(dat, "\t%.10f", (TimeSeries[i].Critical_adunit[j] - Critical_a[j]) + 0.0);
				for (j = 0; j < P.NumAdunits; j++)		fprintf(dat, "\t%.10f", (TimeSeries[i].CritRecov_adunit[j] - CritRecov_a[j]) + 0.0);

				//// incidence
				for (j = 0; j < P.NumAdunits; j++)		fprintf(dat, "\t%.10f", TimeSeries[i].incI_adunit[j] + 0.0);
				for (j = 0; j < P.NumAdunits; j++)		fprintf(dat, "\t%.10f", TimeSeries[i].incMild_adunit[j] + 0.0);
				for (j = 0; j < P.NumAdunits; j++)		fprintf(dat, "\t%.10f", TimeSeries[i].incILI_adunit[j] + 0.0);
				for (j = 0; j < P.NumAdunits; j++)		fprintf(dat, "\t%.10f", incSARI_a[j] + 0.0);
				for (j = 0; j < P.NumAdunits; j++)		fprintf(dat, "\t%.10f", incCritical_a[j] + 0.0);
				for (j = 0; j < P.NumAdunits; j++)		fprintf(dat, "\t%.10f", incCritRecov_a[j] + 0.0);
				for (j = 0; j < P.NumAdunits; j++)		fprintf(dat, "\t%lg", TimeSeries[i].incD_adunit[j] + 0.0);

				//// cumulative incidence
				for (j = 0; j < P.NumAdunits; j++)		fprintf(dat, "\t%.10f", TimeSeries[i].cumMild_adunit[j] + 0.0);
				for (j = 0; j < P.NumAdunits; j++)		fprintf(dat, "\t%.10f", TimeSeries[i].cumILI_adunit[j] + 0.0);
				for (j = 0; j < P.NumAdunits; j++)		fprintf(dat, "\t%.10f", TimeSeries[i].cumSARI_adunit[j] + 0.0);
				for (j = 0; j < P.NumAdunits; j++)		fprintf(dat, "\t%.10f", TimeSeries[i].cumCritical_adunit[j] + 0.0);
				for (j = 0; j < P.NumAdunits; j++)		fprintf(dat, "\t%.10f", TimeSeries[i].cumCritRecov_adunit[j] + 0.0);
				for (j = 0; j < P.NumAdunits; j++)		fprintf(dat, "\t%.10f", TimeSeries[i].cumD_adunit[j] + 0.0);
