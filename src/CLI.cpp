...
void parse_read_file(std::string const& input, std::string& output)
{
	...
	{
    Messages::out(Messages::Error) << input << " is not a file\n";
	}
	output = input;
}
...
void parse_write_dir(std::string const& input, std::string& output)
{
    ...
    Messages::out(Messages::Error) <<
      "Cannot use this prefix, this path already exists as a file: " <<
      input.c_str() << "\n";
	}
...
void parse_integer(std::string const& input, int& output)
{
	try
	{
		...
		if (pos != input.size())
		{
      Messages::out(Messages::Error) << "Detected invalid characters after parsed integer: " << input << "\n";
		}
	}
	catch (const std::invalid_argument& e)
	{
    Messages::out(Messages::Error) << "EINVAL: Expected integer got " << input << "\n";
	}
	catch (const std::out_of_range& e)
	{
    Messages::out(Messages::Error) << "ERANGE: Input integer is out of range. "
      "Expected " << std::numeric_limits<int>::min() << " to " <<
      std::numeric_limits<int>::max() << ".  Got " << input << "\n";
	}
}
...
void parse_double(std::string const& input, double& output)
{
	try
	{
		...
		if (pos != input.size())
		{
      Messages::out(Messages::Error) << "Detected invalid characters after parsed double: " << input << "\n";
		}
	}
	catch (const std::invalid_argument& e)
	{
    Messages::out(Messages::Error) << "EINVAL: Expected double got " << input << "\n";
	}
	catch (const std::out_of_range& e)
	{
    Messages::out(Messages::Error) << "ERANGE: Input double is out of range. "
      "Expected " << std::numeric_limits<double>::min() << " to " <<
      std::numeric_limits<double>::max() << ".  Got " << input << "\n";
	}
}
...
void CmdLineArgs::add_custom_option(std::string const& option, ParserFn func, std::string const& doc)
{
	...
	{
    Messages::out(Messages::Error) << "Duplicate option specified " << option << "\n";
	}
