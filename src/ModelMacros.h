...
#define PLACE_CLOSED(x, y)                                                                         \
	((Places[x][y].close_start_time <= ts) && (Places[x][y].close_end_time > ts))
...
#define HOST_QUARANTINED(x)                                                                        \
	((Hosts[x].quar_comply == 1) &&                                                                  \
	 (Hosts[x].quar_start_time + P.usHQuarantineHouseDuration > ts) &&                               \
	 (Hosts[x].quar_start_time <= ts))
#define HOST_TO_BE_QUARANTINED(x)                                                                  \
	((Hosts[x].quar_start_time + P.usHQuarantineHouseDuration > ts) && (Hosts[x].quar_comply < 2))
#define HOST_ISOLATED(x)                                                                           \
	((Hosts[x].isolation_start_time + P.usCaseIsolationDelay <= ts) &&                               \
	 (Hosts[x].isolation_start_time + P.usCaseIsolationDelay + P.usCaseIsolationDuration > ts))
