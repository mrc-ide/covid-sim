...
void InfectiousAlmostSymptomatic::GetsWorse(int ai, double t, int tn, int run)
{
	...
	Person* a = Hosts + ai;
	a->infectionState = new Case();

}

void InfectiousAlmostSymptomatic::GetsBetter(int ai, double t, int tn, int run)
{
}
...
void InfectiousAlmostSymptomatic::DoILI(int ai, int tn)
{
...
void InfectiousAlmostSymptomatic::DoMild(int ai, int tn)
{
	if (P->DoSeverity) //// shouldn't need this but best be careful.
	{
		Person* a = Hosts + ai;
...
void InfectiousAlmostSymptomatic::DoSARI(int ai, int tn)
{
	if (P->DoSeverity) //// shouldn't need this but best be careful.
	{
		Person* a = Hosts + ai;
...
void InfectiousAlmostSymptomatic::DoCritical(int ai, int tn)
{
	if (P->DoSeverity) //// shouldn't need this but best be careful.
	{
		Person* a = Hosts + ai;
...
void InfectiousAlmostSymptomatic::DoRecoveringFromCritical(int ai, int tn)
{
...
void InfectiousAlmostSymptomatic::DoDeath_FromCriticalorSARIorILI(int ai, int tn)
{
	Person* a = Hosts + ai;
	if (P->DoSeverity)
	{
...
void InfectiousAlmostSymptomatic::DoRecover_FromSeverity(int ai, int tn)
{
...
void InfectiousAlmostSymptomatic::DoDetectedCase(int ai, double t, unsigned short int ts, int tn)
{
...
#pragma omp critical (starttreat)
	...
	if (P->DoHouseholds)
	{
		...
		{
			j1 = Households[Hosts[ai].hh].FirstPerson; j2 = j1 + Households[Hosts[ai].hh].nh;
			if ((!HOST_TO_BE_QUARANTINED_PTR(j1)) || (P->DoHQretrigger))
			{
...
void InfectiousAlmostSymptomatic::DoFalseCase(int ai, double t, unsigned short int ts, int tn)
{
...
void InfectiousAlmostSymptomatic::DoProphNoDelay(int ai, unsigned short int ts, int tn, int nc)
{
	int x, y;
...
void InfectiousAlmostSymptomatic::DoPlaceClose(int i, int j, unsigned short int ts, int tn, int DoAnyway)
{
	...
	if (f2)
	{
		if (P->DoRealSymptWithdrawal)
			for (k = 0; k < Places[i][j].n; k++) //// loop over all people in place.
			{
				...
				{
					if ((!HOST_ABSENT(ai)) && (!HOST_QUARANTINED_PTR(ai)) && (HOST_AGE_YEAR(ai) < P->CaseAbsentChildAgeCutoff)) //// if person is a child and neither absent nor quarantined
					{
						StateT[tn].cumAPCS++;
						if (Hosts[ai].ProbCare < P->CaseAbsentChildPropAdultCarers) //// if child needs adult supervision
						{
							...
							//// in loop below, f true if any household member a) alive AND b) not a child AND c) has no links to workplace (or is absent from work or quarantined).
							for (l = j1; (l < j2) && (!f); l++)
								f = ((abs(Hosts[l].inf) != InfStat_Dead) && (HOST_AGE_YEAR(l) >= P->CaseAbsentChildAgeCutoff) && ((Hosts[l].PlaceLinks[P->PlaceTypeNoAirNum - 1] < 0) || (HOST_QUARANTINED_PTR(l))));
...
void InfectiousAlmostSymptomatic::DoPlaceOpen(int i, int j, unsigned short int ts, int tn)
{
	int k, ai, j1, j2, l, f;
		...
		if (ts < Places[i][j].close_end_time)
		{
			if (P->DoRealSymptWithdrawal)
				for (k = 0; k < Places[i][j].n; k++)
				{
					...
					if (Hosts[ai].ProbCare < P->CaseAbsentChildPropAdultCarers) //// if child needs adult supervision
					{
						if ((HOST_AGE_YEAR(ai) < P->CaseAbsentChildAgeCutoff) && (!HOST_QUARANTINED_PTR(ai)))
						{
							j1 = Households[Hosts[ai].hh].FirstPerson; j2 = j1 + Households[Hosts[ai].hh].nh;
							f = 0;
							for (l = j1; (l < j2) && (!f); l++)
...
int InfectiousAlmostSymptomatic::DoVacc(int ai, unsigned short int ts)
{
	int x, y;
...
void InfectiousAlmostSymptomatic::DoVaccNoDelay(int ai, unsigned short int ts)
{
	int x, y;
...
void InfectiousAlmostSymptomatic::DoTreatCase(int ai, unsigned short int ts, int tn)
{
	int x, y;
...
void InfectiousAlmostSymptomatic::DoProph(int ai, unsigned short int ts, int tn)
{
