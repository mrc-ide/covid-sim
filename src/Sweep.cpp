...
void TravelReturnSweep(double t)
{
	...
	if (!(P.DoAirports && P.HotelPlaceType < P.PlaceTypeNum))
		ERR_CRITICAL("DoAirports || HotelPlaceType not set\n");
				...
				for (int k = n - 1; k >= 0; k--)
				{
					...
					if (Hosts[i].Travelling == l)
					{
						...
						/*						if((n<0)||(Places[P.HotelPlaceType][j].members[n]<0)||(Places[P.HotelPlaceType][j].members[n]>=P.PopSize))
							 ...
							 %i\n",j,k,n,Places[P.HotelPlaceType][j].members[n]);ner++;} else if((k<0)||(k>n))
													{fprintf(stderr,"@ %i %i %i
							 %i\n",j,k,n,Places[P.HotelPlaceType][j].members[n]);ner++;} else
						*/
						if (k != n)
						{
						...
						{
							ner++;
							fprintf(stderr_shared, "(%i %i) ", j, Hosts[i].PlaceLinks[P.HotelPlaceType]);
						}
...
void TravelDepartSweep(double t)
{
	...
	if (floor(1 + t - P.TimeStep) != floor(1 + t))
	{
		bm = ((P.DoBlanketMoveRestr) && (t >= P.MoveRestrTimeStart) &&
					(t < P.MoveRestrTimeStart + P.MoveRestrDuration));
		...
		for (int tn = 0; tn < P.NumThreads; tn++)
			for (int i = tn; i < P.Nairports; i += P.NumThreads)
				if ((Airports[i].total_traffic > 0) && (Airports[i].num_mcell > 0))
				{
					...
					if ((t > P.AirportCloseTimeStart) &&
							(t < P.AirportCloseTimeStart + P.AirportCloseTimeStartBase))
					...
					while (j < n)
					{
						...
						while (Airports[i].DestMcells[l].prob < s)
							l++;
						l = Airports[i].DestMcells[l].id;
						int k = (int)(ranf_mt(tn) * ((double)Mcells[l].n));
						int i2 = Mcells[l].members[k];
						if ((abs(Hosts[i2].inf) < InfStat_InfectiousAsymptomaticNotCase) &&
								(Hosts[i2].inf != InfStat_Case))
						{
...
#pragma omp critical
								...
								if (!f2)
								{
									...
									while (Airports[i].prop_traffic[l] < s)
										l++;
									k = Airports[i].conn_airports[l];
									if (bm)
									{
										if (dist2_raw(Airports[i].loc.x, Airports[i].loc.y, Airports[k].loc.x,
																	Airports[k].loc.y) > P.MoveRestrRadius2)
										{
...
#pragma omp critical
									...
									if (!f2)
									{
										...
										do
										{
											...
											while (Airports[k].DestPlaces[m].prob < s)
												m++;
...
#pragma omp critical
											...
											if (!f)
											{
												...
												d2 = (d + P.InvJourneyDurationDistrib[(int)(ranf_mt(tn) * 1024.0)]) %
														 MAX_TRAVEL_TIME;
...
#pragma omp critical
		...
		nl =
				((double)P.PlaceTypeMeanSize[P.HotelPlaceType]) * P.HotelPropLocal / P.MeanLocalJourneyTime;
		nsk = 0;
#pragma omp parallel for reduction(+:nld,nsk) schedule(static,1) default(none) \
		...
		for (int tn = 0; tn < P.NumThreads; tn++)
			...
			{
				int c = ((int)(Places[P.HotelPlaceType][i].loc.x / P.in_cells_.width)) * P.nch +
								((int)(Places[P.HotelPlaceType][i].loc.y / P.in_cells_.height));
				...
				for (int j = 0; j < n; j++)
				{
					...
					do
					{
						...
						if (m < (ct->S + ct->L))
						{
							...
							int d2 = HOST_AGE_GROUP(i2);
							int f3 = 0;
							if ((Hosts[i2].Travelling == 0) &&
									((P.RelativeTravelRate[d2] == 1) || (ranf_mt(tn) < P.RelativeTravelRate[d2])))
							{
#pragma omp critical
								{
									if (Hosts[i2].PlaceLinks[P.HotelPlaceType] == -1)
							...
							{
								double s2 =
										dist2_raw(Households[Hosts[i2].hh].loc.x, Households[Hosts[i2].hh].loc.y,
															Places[P.HotelPlaceType][i].loc.x, Places[P.HotelPlaceType][i].loc.y);
								int f2 = 1;
...
#pragma omp critical
									...
									{
										d2 = (d + P.InvLocalJourneyDurationDistrib[(int)(ranf_mt(tn) * 1024.0)]) %
												 MAX_TRAVEL_TIME;
...
void InfectSweep(double t,
								 int run) // added run number as argument in order to record it in event log
{
	//// This function takes the day number (t) and run number (run) as inputs. It loops over infected
	/// people, and decides whom to infect. Structure is 1) #pragma loop over all cells then 1a)
	/// infectious people, which chooses who they will infect, adds them to a queue / Next 2) #pragma
	/// loop infects those people from queue (using DoInfect function). This is to avoid race
	/// conditions. / Loop 1a) calculates the force of infection exerted by each infected person on
	...
	//// various quantities of force of infection, including "infectiousness" and "susceptibility"
	/// components
	double s;							// household Force Of Infection (FOI) on fellow household member, then place
												// susceptibility, then random number for spatial infections allocation* / ;
...
#pragma omp parallel for private(n, f, f2, s, s2, s3, s4, s5, s6, cq, ci, s3_scaled, s4_scaled)    \
		schedule(static, 1) default(none)                                                              \
				shared(t, P, CellLookup, Hosts, AdUnits, Households, Places, SamplingQueue, Cells, Mcells, \
							 StateT, hbeta, sbeta, seasonality, ts, fp, bm, stderr_shared)
	for (int tn = 0; tn < P.NumThreads; tn++)
		for (int b = tn; b < P.NCP;
				 b += P.NumThreads) //// loop over (in parallel) all populated cells. Loop 1)
		{
			...
			for (int j = 0; j < c->I; j++)
			{
				...
				ci = c->infected[j];
				//// get person si from Hosts (array of people) corresponding to ci, using pointer
				/// arithmetic.
				Person *si = Hosts + ci;
				...
				// AND Day number (t) is greater than the start day for contact tracing in this
				// administrative unit (ie. contact tracing has started) AND Day number (t) is less than the
				// end day for contact tracing in this administrative unit (ie. contact tracing has not
				// ended) AND the selected host is a digital contact tracing user otherwise fct = 0
				int fct = ((P.DoDigitalContactTracing) &&
									 (t >= AdUnits[Mcells[si->mcell].adunit].DigitalContactTracingTimeStart) &&
									 (t < AdUnits[Mcells[si->mcell].adunit].DigitalContactTracingTimeStart +
														P.DigitalContactTracingPolicyDuration) &&
									 (Hosts[ci].digitalContactTracingUser ==
				...
				// hbeta =  seasonality * fp * P.HouseholdTrans or 0 depending on whether households
				// functionality is on or off - see start of function
				...
				if (hbeta > 0)
				{
					...
					if ((Households[si->hh].nh > 1) && (!si->Travelling))
					{
						...
						// using the CalcHouseInf function on the selected cell and timestamp at start of
						// current day then scaling by hbeta
						s3 = hbeta * CalcHouseInf(ci, ts);

						// Test if any of the individuals in the selected persons household are absent from
						// places f=0 means noone absent, f=1 means at least one absent
						...
						{ //// loop over people in household
							...
							{ //// loop over place types
								...
								{ //// if person in household has any sort of link to place type
						...
						// if individuals in the household are absent from places (ie. f==1 from test
						// immediately above), scale up the infectiousness (s3) of the household
						if (f)
						{
							s3 *= P.PlaceCloseHouseholdRelContact;
						} /* NumPCD++;}*/ //// if people in your household are absent from places, person si/ci
															/// is more infectious to them, as they spend more time at home.

						// Loop from l (the index of the first person in the household) to m-1 (the index of the
						// last person in the household) ie. loop over everyone in the household
						for (int i3 = l; i3 < m;
								 i3++) //// loop over all people in household (note goes from l to m - 1)
						{
							if ((Hosts[i3].inf == InfStat_Susceptible) &&
									(!Hosts[i3].Travelling)) //// if people in household uninfected/susceptible and
																					 /// not travelling
							{
								s = s3 * CalcHouseSusc(i3, ts, ci,
																			 tn); //// FOI ( = infectiousness x susceptibility) from
																						/// person ci/si on fellow household member i3
				...
				if (P.DoPlaces)
				{
					...
					if (!HOST_ABSENT(ci))
					{
						...
						{
							// select link (l) between selected host (si) and place from si's placelinks to place
							// type k
							int l = si->PlaceLinks[k];
							if (l >= 0) //// l>=0 means if place type k is relevant to person si. (Now allowing
													/// for partial attendance).
							{
								...
								// else if movement restrictions in effect in either household microcell or place
								// microcell
								else if ((mi->moverest != mp->moverest) &&
												 ((mi->moverest == 2) || (mp->moverest == 2)))
								{
									// multiply infectiousness of place by movement restriction effect
									s3 *= P.MoveRestrEffect;
								}
								...
								if ((k != P.HotelPlaceType) && (!si->Travelling))
								{
									...
									// if contact tracing is in place, s4_scaled is s3  *
									// P.ScalingFactorPlaceDigitalContacts in either case s4_scaled is capped at 1
									...
									else if (s4_scaled >= 1) //// if place infectiousness above threshold, consider
																					 /// everyone in group a potential infectee...
									{
										// set n to be number of people in group in place k,l
										n = Places[k][l].group_size[i2];
									}
									else //// ... otherwise randomly sample (from binomial distribution) number of
											 /// potential infectees in this place.
									{
									...
									if (n > 0)
									{
										...
										SampleWithoutReplacement(
												tn, n,
												Places[k][l].group_size[i2]); //// changes thread-specific SamplingQueue.
									}
									...
									for (int m = 0; m < n; m++)
									{
										...
										int i3 =
												Places[k][l].members[Places[k][l].group_start[i2] + SamplingQueue[tn][m]];
										// calculate place susceptbility based on infectee (i3), place type (k),
										// timestep (ts) cell (ci) and thread number (tn)
										s = CalcPlaceSusc(i3, k, ts, ci, tn);

										// ** add potential infectees to digital contact tracing queue**
										// if contact tracing in place (fct) AND potential infectee (i3) is a contact
										// tracing user AND potential infectee != infectionus person ci AND potential
										// infectee (i3) isn't absent
												...
												(!HOST_ABSENT(i3)))
										{
											...
											// if random number < s6
											// AND number of contacts of ci(!) is less than maximum digital contact to
											// trace
											if ((Hosts[ci].ncontacts < P.MaxDigitalContactsToTrace) && (ranf_mt(tn) < s6))
											{
												...
												{
													// find adunit for contact and add both contact and infectious host to
													// lists - storing both so I can set times later.
										...
										//
										if ((Hosts[i3].inf == InfStat_Susceptible) &&
												(!HOST_ABSENT(i3))) //// if person i3 uninfected and not absent.
										{
											...
											if (bm)
											{
												...
												if ((dist2_raw(Households[Hosts[i3].hh].loc.x,
																			 Households[Hosts[i3].hh].loc.y, Places[k][l].loc.x,
																			 Places[k][l].loc.y) > P.MoveRestrRadius2))
												{
											...
											if ((s == 1) || (ranf_mt(tn) < s))
											{
												...
												if ((StateT[tn].n_queue[cq] <
														 P.InfQueuePeakLength)) //(Hosts[i3].infector==-1)&&
												{
													// false positive
													if ((P.FalsePositiveRate > 0) && (ranf_mt(tn) < P.FalsePositiveRate))
														StateT[tn].inf_queue[cq][StateT[tn].n_queue[cq]++] = {-1, i3, -1};
								...
								if ((k == P.HotelPlaceType) || (!si->Travelling))
								{
									s3 *= P.PlaceTypePropBetweenGroupLinks[k] * P.PlaceTypeGroupSizeParam1[k] /
												((double)Places[k][l].n);
									if (s3 > 1)
										s3 = 1;
									// if contact tracing in place, multiply s3_scaled = s3*scalingfactor, otherwise
									// s3_scaled = s3
									...
									// if more than 0 potential infectees, pick n hosts from the hotel and add to
									// sampling queue
									if (n > 0)
										SampleWithoutReplacement(tn, n, Places[k][l].n);
									...
									for (int m = 0; m < n; m++)
									{
										...
										// these are all place group contacts to be tracked for digital contact tracing
										// - add to StateT queue for contact tracing if infectee is also a user, add
										// them as a contact

										// if contact tracing in place AND potential infectee i3 is a contact tracing
										// user AND i3 isn't absent AND i3 isn't ci (suspect this should be si)
										if ((fct) && (Hosts[i3].digitalContactTracingUser) && (ci != i3) &&
												(!HOST_ABSENT(i3)))
										{
										...
										if ((Hosts[i3].inf == InfStat_Susceptible) && (!HOST_ABSENT(i3)))
										{
											...
											if (bm)
											{
												// if potential infectees household is farther away from hotel than
												// restriction radius
												if ((dist2_raw(Households[Hosts[i3].hh].loc.x,
																			 Households[Hosts[i3].hh].loc.y, Places[k][l].loc.x,
																			 Places[k][l].loc.y) > P.MoveRestrRadius2))
											...
											// else if movement restrictions are in place in potential infectee's cell or
											// hotel's cell
											else if ((mt->moverest != mp->moverest) &&
															 ((mt->moverest == 2) || (mp->moverest == 2)))
											{
											...
											// is susceptibility is 1 (ie infect everyone) or random number is less than
											// susceptibility
											if ((s == 1) || (ranf_mt(tn) < s))
											{
														 ...
														 P.InfQueuePeakLength)) //(Hosts[i3].infector==-1)&&
												{
													...
													else
													{
														short int infect_type =
																2 + k + NUM_PLACE_TYPES +
																INFECT_TYPE_MASK * (1 + si->infect_type / INFECT_TYPE_MASK);
				...
				//// ie sum spatial infectiousness over all infected people, the infections from which are
				/// allocated after loop over infected people.
				if (sbeta > 0)
				{
					if (si->Travelling) //// if host currently away from their cell, they cannot add to their
															/// cell's spatial infectiousness.
					{
						s2 = 0;
						f = 0;
					}
					else
					{
						...
						// if do digital contact tracing, scale up spatial infectiousness of infectives who are
						// using the app and will be detected
						if (fct)
						{
							s2 *= P.ScalingFactorSpatialDigitalContacts;
						}
					}
					// test if selected person si is linked to a place that is closed, f=0 means no links to
					// closed places, otherwise f=1
					...
					if (P.DoPlaces)
					{
						...
						for (int i3 = 0; (i3 < P.PlaceTypeNum) && (!f); i3++)
						{
							if (si->PlaceLinks[i3] >= 0) //// if person has a link to place of type i3...
							{
								...
								f = PLACE_CLOSED(
										i3, si->PlaceLinks[i3]); //// find out if that place of type i3 is closed.
							}
						}
					} // if doing places

					if ((f) &&
							(HOST_ABSENT(ci))) //// if place is closed and person is absent then adjust the
																 /// spatial infectiousness (similar logic to household
																 /// infectiousness: place closure affects spatial infectiousness
					{
			...
			if (s5 > 0) //// if spatial infectiousness positive
			{
				...
				n = (int)ignpoi_mt(s5 * sbeta * ((double)c->tot_prob),
													 tn); //// number people this cell's population might infect elsewhere.
																/// poisson random number based on spatial infectiousness s5, sbeta
																///(seasonality) and this cell's "probability" (guessing this is a
																/// function of its population and geographical size).
				// i2 = number of infectious people in cell c
				int i2 = c->I;

				if (n > 0) //// this block normalises cumulative infectiousness cell_inf by person. s5 is
									 /// the total cumulative spatial infectiousness. Reason is so that infector can
									 /// be chosen using ranf_mt, which returns random number between 0 and 1.
				{
					//// normalise by cumulative spatial infectiousness.
					for (int j = 0; j < i2 - 1; j++)
						StateT[tn].cell_inf[j] /= ((float)s5);
					//// does same as the above loop just a slightly faster calculation. i.e.
					/// StateT[tn].cell_inf[i2 - 1] / s5 would equal 1 or -1 anyway.
					StateT[tn].cell_inf[i2 - 1] = (StateT[tn].cell_inf[i2 - 1] < 0) ? -1.0f : 1.0f;
				}

				//// loop over infections to dole out. roughly speaking, this determines which infectious
				/// person in cell c infects which person elsewhere.
				for (int k = 0; k < n; k++)
				{
					...
					//// roughly speaking, this determines which infectious person in cell c infects which
					/// person elsewhere
					else
					{
						int m;
						s = ranf_mt(tn); ///// choose random number between 0 and 1
						j = m = i2 / 2;	 ///// assign j and m to be halfway between zero and number of infected
														 /// people i2 = c->I.
						f = 1;
						do
						{
							if (m > 1)
								m /= 2; //// amount m to change j by reduced by half. Looks like a binary search.
												/// Basically saying, keep amending potential infector j until either j less
												/// than zero or more than number of infected people until you find j s.t.
												/// spatial infectiousness "matches" s.
					...
					f = (StateT[tn].cell_inf[j] <
							 0); //// flag for whether infector j had their place(s) closed. <0 (true) = place
									 /// closed / >=0 (false) = place not closed. Set in if (sbeta > 0) part of loop
									 /// over infectious people.
					// ci is the index of the jth infectious person in the cell
					ci = c->infected[j];
					// si is the jth selected person in the cell
					Person *si = Hosts + ci;

					// calculate flag (fct) for digital contact tracing here at the beginning for each
					// individual infector
					int fct = ((P.DoDigitalContactTracing) &&
										 (t >= AdUnits[Mcells[si->mcell].adunit].DigitalContactTracingTimeStart) &&
										 (t < AdUnits[Mcells[si->mcell].adunit].DigitalContactTracingTimeStart +
					...
					do
					{
						...
						while (c->cum_trans[l] < s)
							l++;
						// selecte the cell corresponding to l
						Cell *ct = CellLookup[l];

						///// pick random person m within susceptibles of cell ct (S0 initial number
						/// susceptibles within cell).
						int m = (int)(ranf_mt(tn) * ((double)ct->S0));
						int i3 = ct->susceptible[m];

						s2 = dist2(Hosts + i3,
											 Hosts + ci); /// calculate distance squared between this susceptible person
																		/// and person ci/si identified earlier
						...
						if ((ranf_mt(tn) >= s) ||
								(abs(Hosts[i3].inf) ==
								 InfStat_Dead)) //// if rejected, or infectee i3/m already dead, ensure do-while
																/// evaluated again (i.e. choose a new infectee).
						{
						...
						{
							//// if potential infectee not travelling, and either is not part of cell c or doesn't
							/// share a household with infector.
							if ((!Hosts[i3].Travelling) && ((c != ct) || (Hosts[i3].hh != si->hh)))
							{
								...
								// so this person is a contact - but might not be infected. if we are doing digital
								// contact tracing, we want to add the person to the contacts list, if both are
								// users
								if (fct)
								{
									...
									if (Hosts[i3].digitalContactTracingUser && (ci != i3))
									{
												...
												(ranf_mt(tn) < s * P.ProportionDigitalContactsIsolate))
										{
											...
											if ((StateT[tn].ndct_queue[ad] < AdUnits[ad].n))
											{
												// find adunit for contact and add both contact and infectious host to lists
												// - storing both so I can set times later.
												StateT[tn].dct_queue[ad][StateT[tn].ndct_queue[ad]++] = {i3, ci, ts};
											}
											else
											{
								...
								if (m < ct->S) // only bother trying to infect susceptible people
								{
									...
									{
										if ((dist2_raw(Households[si->hh].loc.x, Households[si->hh].loc.y,
																	 Households[Hosts[i3].hh].loc.x,
																	 Households[Hosts[i3].hh].loc.y) > P.MoveRestrRadius2))
											s *= P.MoveRestrEffect;
									}
										...
										s *= P.MoveRestrEffect;
									if ((!f) &&
											(HOST_ABSENT(i3))) //// if infector did not have place closed, loop over place
																				 /// types of infectee i3 to see if their places had closed.
																				 /// If they had, amend their susceptibility.
									{
										...
										if (f2)
										{
											s *= P.PlaceCloseSpatialRelContact;
										} /* NumPCD++;} */
										f2 = 0;
									}
							...
							}		// //// if potential infectee not travelling, and either is not part of cell c or
									// doesn't share a household with infector
						}			// infectee isn't dead
					} while (f2);
...
#pragma omp parallel for schedule(static, 1) default(none) shared(t, run, P, StateT, Hosts, ts)
...
#pragma omp parallel for schedule(static, 1) default(none) shared(P, Places, Hosts, i, j, ht)
					for (int tn = 0; tn < P.NumThreads; tn++)
						...
						{
							if ((P.HolidayEffect[j] < 1) &&
									((P.HolidayEffect[j] == 0) || (ranf_mt(tn) >= P.HolidayEffect[j])))
							{
								...
								{
									if (Hosts[Places[j][k].members[ci]].absent_start_time > l)
										Hosts[Places[j][k].members[ci]].absent_start_time = (unsigned short)l;
									if (Hosts[Places[j][k].members[ci]].absent_stop_time < b)
										Hosts[Places[j][k].members[ci]].absent_stop_time = (unsigned short)b;
								}
							}
						}
				}
...
#pragma omp parallel for schedule(static, 1) default(none)                                         \
	...
	for (int tn = 0; tn < P.NumThreads; tn++)				 //// loop over threads
		for (int b = tn; b < P.NCP; b += P.NumThreads) //// loop/step over populated cells
		{
			...
			for (int j = ((int)c->L - 1); j >= 0;
					 j--) //// loop backwards over latently infected people, hence it starts from L - 1 and
								/// goes to zero. Runs backwards because of pointer swapping?
				if (ts >= Hosts[c->latent[j]].latent_time) //// if now after time at which person became
																									 /// infectious (latent_time a slight misnomer).
					DoIncub(c->latent[j], ts, tn, run); //// move infected person from latently infected (L)
																							/// to infectious (I), but not symptomatic
			// StateT[tn].n_queue[0] = StateT[tn].n_queue[1] = 0;
			for (int j = c->I - 1; j >= 0; j--) ///// loop backwards over Infectious people. Runs
																					/// backwards because of pointer swapping?
			{
				int ci = c->infected[j]; //// person index
				Person *si = Hosts + ci; //// person

				unsigned short int tc; //// time at which person becomes case (i.e. moves from infectious
															 /// and asymptomatic to infectious and symptomatic).
				/* Following line not 100% consistent with DoIncub. All severity time points (e.g. SARI
				 * time) are added to latent_time, not latent_time + ((int)(P.LatentToSymptDelay /
				 * P.TimeStep))*/
				tc = si->latent_time +
						 ((int)(P.LatentToSymptDelay /
										P.TimeStep));					//// time that person si/ci becomes case (symptomatic)...
				if ((P.DoSymptoms) && (ts == tc)) //// ... if now is that time...
					DoCase(ci, t, ts,
								 tn); //// ... change infectious (but asymptomatic) person to infectious and
											/// symptomatic. If doing severity, this contains DoMild and DoILI.

				if (P.DoSeverity)
				{
					if (ts >= si->SARI_time)
						DoSARI(ci, tn); //// see if you can dispense with inequalities by initializing
														/// SARI_time, Critical_time etc. to USHRT_MAX
					if (ts >= si->Critical_time)
						DoCritical(ci, tn);
					if (ts >= si->RecoveringFromCritical_time)
						DoRecoveringFromCritical(ci, tn);
				...
				// Adding code to assign recovery or death when leaving the infectious class: ggilani -
				// 22/10/14
				if (ts >= si->recovery_or_death_time)
				{
					...
					{
						DoRecover(ci, tn, run);
						// StateT[tn].inf_queue[0][StateT[tn].n_queue[0]++] = ci; //// add them to end of 0th
						// thread of inf queue. Don't get why 0 here.
					}
					...
					// once host recovers, will no longer make contacts for contact tracing - if we are doing
					// contact tracing and case was infectious when contact tracing was active, increment
					// state vector
					if ((P.DoDigitalContactTracing) &&
							(Hosts[ci].latent_time >=
							 AdUnits[Mcells[Hosts[ci].mcell].adunit].DigitalContactTracingTimeStart) &&
							(Hosts[ci].recovery_or_death_time <
							 AdUnits[Mcells[Hosts[ci].mcell].adunit].DigitalContactTracingTimeStart +
									 P.DigitalContactTracingPolicyDuration) &&
							(Hosts[ci].digitalContactTracingUser == 1) && (P.OutputDigitalContactDist))
					{
						if (Hosts[ci].ncontacts > MAX_CONTACTS)
							Hosts[ci].ncontacts = MAX_CONTACTS;
...
void DigitalContactTracingSweep(double t)
{
	 ...
	 *
	 * Purpose: to update and count the number of people in each admin unit who are being digitally
	 * contact traced each day and remove those who no longer need to be traced
			...
			if (t >= AdUnits[i].DigitalContactTracingTimeStart)
			{
				for (int j = 0; j < P.NumThreads; j++)
				{
					for (int k = 0; k < StateT[j].ndct_queue[i];)
					{
						...
						// these are calculated here for each time step instead of InfectSweep when contact
						// event is added as trigger times will be updated for asymptomatic cases detected by
						// testing.
						...
						// this condition is only ever met when a symptomatic case is detected in DoDetectedCase
						// and is not already an index case. If they have already been made an index case due to
						// testing, then this won't occur again for them.
						if (infector == -1)
						{
							// i.e. this is an index case that has been detected by becoming symptomatic and added
							// to the digital contact tracing queue
							dct_start_time =
									Hosts[contact]
											.dct_trigger_time; // trigger time for these cases is set in DoIncub and
																				 // already accounts for delay between onset and isolation
							dct_end_time = dct_start_time + (unsigned short int)(P.LengthDigitalContactIsolation *
																																	 P.TimeStepsPerDay);
						}
						else // We are looking at actual contact events between infectious hosts and their
								 // contacts.
						{
							// trigger times are either set in DoDetectedCase or in the loop below (for
							// asymptomatic and presymptomatic cases that are picked up via testing If the
							// contact's index case has a trigger time that means that they have been detected,
							// and we can calculate start and end isolation times for the contact.
							if (Hosts[infector].dct_trigger_time < (USHRT_MAX - 1))
							{
								...
								{
									// if the contact time was made before or at the same time as detection, use the
									// trigger time instead
									dct_start_time =
											Hosts[infector].dct_trigger_time +
											(unsigned short int)(P.DigitalContactTracingDelay * P.TimeStepsPerDay);
								}
								dct_end_time =
										dct_start_time +
							...
							{
								dct_start_time =
										USHRT_MAX - 1; // for contacts of asymptomatic or presymptomatic cases - they
																	 // won't get added as their index case won't know that they are
																	 // infected (unless explicitly tested)
								// but we keep them in the queue in case their index case is detected as the contact
								// of someone else and gets their trigger time set set dct_end_time to recovery time
								// of infector, in order to remove from queue if their infector isn't detected
								// before they recover.
						...
						{
							// if the host has been detected due to being symptomatic, they are now an index case
							// - set this variable now. For index cases detected by testing, this will be set on
							// testing
							if ((infector == -1) && (Hosts[contact].index_case_dct ==
																			 0)) // don't really need the second condition as the first
																					 // should only be true when the second isn't (due to how
																					 // this contact is logged in DoDetectedCase)
							{
							...
							{
								// move into the contact tracing list for that admin unit, set start and end times,
								// update flag and remove from queue
								if (AdUnits[i].ndct < AdUnits[i].n) // AdUnits[i].n is length of queue
								{
									...
									// At this point, we do testing on index cases who have been picked up on symptoms
									// alone, in order to figure out whether and when to remove their contacts (if
									// P.RemoveContactsOfNegativeIndexCase). It's much harder to do it in the next
									// loop as we don't have all the information about the contact event there and
									// would need to loop over all contacts again to look for their index case This
									// would cause race conditions due to having a loop over adunits within threaded
									// loop over admin units Only set test times if P.DoDCTTest. If P.DoDCTTest==0,
									// but we are finding contacts of contacts, we check to see if contacts should
									// become index cases every day they are in isolation.
									if (P.DoDCTTest)
									{
										if (Hosts[contact].index_case_dct == 1)
										{
											// set testing time (which has a different delay to contact testing delay),
											// but no need to set index_case link
											Hosts[contact].dct_test_time =
													dct_start_time +
													(unsigned short int)(P.DelayToTestIndexCase * P.TimeStepsPerDay);
											// if host is infectious at test time
											if ((Hosts[contact].dct_test_time >= Hosts[contact].latent_time) &&
													(Hosts[contact].dct_test_time < Hosts[contact].recovery_or_death_time))
											{
												// if false negative, remove from queue by setting the end time to the test
												// time
												if ((P.SensitivityDCT == 0) ||
														((P.SensitivityDCT < 1) && (ranf_mt(tn) >= P.SensitivityDCT)))
												{
													Hosts[contact].dct_end_time = Hosts[contact].dct_test_time;
													// set index_dct_flag to 2 to indicate that contacts should be removed, if
													// we are removing based on negative test result of index case
													if (P.RemoveContactsOfNegativeIndexCase)
														Hosts[contact].index_case_dct = 2;
												}
											}
											...
											else
											{
												...
												if ((P.SpecificityDCT == 1) ||
														((P.SpecificityDCT > 0) && (ranf_mt(tn) < P.SpecificityDCT)))
												{
													// again mark them to be removed from list at test time rather than
													// end_time, and change index_case_dct flag
													Hosts[contact].dct_end_time = Hosts[contact].dct_test_time;
													if (P.RemoveContactsOfNegativeIndexCase)
														Hosts[contact].index_case_dct = 2;
												}
										...
										{
											// if their infector is set to be removed from the list at test time, and
											// their contacts will also be removed at this stage
											if ((Hosts[infector].index_case_dct == 2) &&
									...
									StateT[j].dct_queue[i][StateT[j].ndct_queue[i] - 1] = {contact, infector,
																																				 contact_time};
							...
							else if (Hosts[contact].digitalContactTraced == 1)
							{
								if (P.DoDCTTest)
								{
									// if case has been detected due to being symptomatic, then we will update their
									// testing time if they would be tested earlier based on being an index case as
									// opposed to being a contact of another case If they are already being contact
									// traced and testing is on, they should have been set a test_time
									if ((Hosts[contact].index_case_dct == 1) &&
											(Hosts[contact].dct_test_time >
											 (dct_start_time +
												(unsigned short int)(P.DelayToTestIndexCase * P.TimeStepsPerDay))))
									{
										Hosts[contact].dct_test_time =
												dct_start_time +
												(unsigned short int)(P.DelayToTestIndexCase * P.TimeStepsPerDay);
										// update end time (which is always at least equal to, but may be later that the
										// current one)
										Hosts[contact].dct_end_time = dct_end_time;
										// check to see if test will be negative, if so, tag them for early removal and
										// update index_dct_flag
										if ((Hosts[contact].dct_test_time >= Hosts[contact].latent_time) &&
												(Hosts[contact].dct_test_time < Hosts[contact].recovery_or_death_time))
										{
									...
									{
										// we don't want to remove this contact if they are also linked to another case
										// - their testing time shouldn't change. but we'll only extend their end time
										// if they wouldn't potentially be removed by having a negative contact
										if ((!P.RemoveContactsOfNegativeIndexCase) ||
												((P.RemoveContactsOfNegativeIndexCase) &&
												 (Hosts[infector].index_case_dct == 1)))
										{
											// extend end time
											Hosts[contact].dct_end_time = dct_end_time;
										}
										// otherwise if contact would have been removed if they didn't have another
										// contact, we keep their original end time
									}
								}
								else
								{
									// just extend the isolation end time, but we're not going to update testing time
									// or as we still want the testing time to be dependent on the earlier contact.
									Hosts[contact].dct_end_time =
											dct_end_time; // we could choose to not extend the time for cases who are
																		// index cases. If they are tested and are negative, they'd be
																		// removed earlier anyway. If positive, they will stay isolated
																		// for a bit longer
								}
						...
						// if contact of an asymptomatic host has passed the recovery time of their asymptomatic
						// index, they would no longer be identified by testing of their index case - remove
						// from the queue so they don't stay here forever
...
#pragma omp parallel for schedule(static, 1) default(none) shared(t, P, AdUnits, Hosts, ts)
	for (int tn = 0; tn < P.NumThreads; tn++)
	{
		for (int i = tn; i < P.NumAdunits; i += P.NumThreads)
		{
			if (t >= AdUnits[i].DigitalContactTracingTimeStart)
			{
				for (int j = 0; j < AdUnits[i].ndct;)
				{
					...
					if (P.DoDCTTest)
					{
						...
						{
							// if host is positive
							if ((abs(Hosts[contact].inf) == 2) ||
									(Hosts[contact].inf == -1)) // either asymptomatic infectious, symptomatic
																							// infectious or presymptomatic infectious
							{
								...
								else if (P.FindContactsOfDCTContacts)
								{
									...
									Hosts[contact].dct_trigger_time =
											ts + 1; // added the +1 here so that if there are no delays, the contacts will
															// still get picked up correctly
									// if they are asymptomatic, i.e. specifically if they have inf flag 2, call
									// DoDetectedCase in order to trigger HQ and PC too.
									if (Hosts[contact].inf == 2)
									{
										DoDetectedCase(contact, t, ts, tn);
										Hosts[contact].detected = 1;
										Hosts[contact].detected_time = ts;
									}
								}
							}
							...
							else
							{
								...
								// can't track contacts of false positives as they don't make any contacts in
								// InfectSweep
							}
						}
					}
					...
					{
						// check every day to see if contacts become index cases - but they have to be
						// infectious. Otherwise we could set the trigger time and cause their contacts to be
						// traced when they are not being traced themselves.
						if ((Hosts[contact].index_case_dct == 0) &&
								((abs(Hosts[contact].inf) == 2) || (Hosts[contact].inf == -1)))
						// if ((Hosts[contact].dct_test_time == ts) && (Hosts[contact].index_case_dct == 0) &&
						// ((abs(Hosts[contact].inf) == 2) || (Hosts[contact].inf == -1)))
						{
							// set them to be an index case
...
{
	///// function loops over microcells to decide which cells are treated (either with treatment,
	/// vaccine, social distancing, movement restrictions etc.)
	...
	if (P.DoGlobalTriggers)
	{
		if (P.DoPerCapitaTriggers)
			global_trig = (int)floor(((double)State.trigDetectedCases) * P.GlobalIncThreshPop /
															 ((double)P.PopSize));
		else
	...
	if ((P.DoPlaces) && (t >= P.TreatTimeStart) &&
			(t < P.TreatTimeStart + P.TreatPlaceGeogDuration) && (State.cumT < P.TreatMaxCourses))
	{
		tstf =
...
#pragma omp parallel for private(f) reduction(+:f1) schedule(static,1) default(none) \
			...
			for (int j = 0; j < P.PlaceTypeNum; j++)
			{
				for (int k = 0; k < StateT[i].np_queue[j]; k++)
				{
					...
					if (P.DoPlaceGroupTreat)
					{
						...
						for (int m = ((int)Places[j][l].group_start[f]);
								 m < ((int)(Places[j][l].group_start[f] + Places[j][l].group_size[f])); m++)
						{
							/*							if((Places[j][l].members[m]<0)||(Places[j][l].members[m]>P.PopSize-1))
															fprintf(stderr,"\n*** npq=%i gn=%i h=%i m=%i j=%i l=%i f=%i s=%i n=%i
								 ***\n", StateT[i].np_queue[j], Places[j][l].n, Places[j][l].members[m], m,j,l,f,
																(int) Places[j][l].group_start[f],
																(int) Places[j][l].group_size[f]);
														else
							*/
							if ((!HOST_TO_BE_TREATED(Places[j][l].members[m])) &&
									((P.TreatPlaceTotalProp[j] == 1) || (ranf_mt(i) < P.TreatPlaceTotalProp[j])))
	...
	///// block vaccinates everyone in mass vaccination queue. Don't know why loop is done twice
	///(although State.mvacc_cum is reset at end so will relate to that)
	if ((P.DoMassVacc) && (t >= P.VaccTimeStart))
		for (int j = 0; j < 2; j++)
		{
			...
			if (m > State.n_mvacc)
				m = State.n_mvacc;
#pragma omp parallel for schedule(static, 1000) default(none) shared(State, m, ts)
			for (int i = State.mvacc_cum; i < m; i++)
				DoVacc(State.mvacc_queue[i], ts);
			State.mvacc_cum = m;
		}
	if ((t >= P.TreatTimeStart) || (t >= P.VaccTimeStartGeo) || (t >= P.PlaceCloseTimeStart) ||
			(t >= P.MoveRestrTimeStart) || (t >= P.SocDistTimeStart) ||
			(t >= P.KeyWorkerProphTimeStart)) // changed this to start time geo
	{
		...
		tspf = (unsigned short int)ceil(P.TimeStepsPerDay *
																		(t + P.PlaceCloseDelayMean + P.PlaceCloseDuration));
...
#pragma omp parallel for private(f2,f3,f4,r) reduction(+:f) schedule(static,1) default(none) \
		...
		for (int tn = 0; tn < P.NumThreads; tn++)
			for (int bs = tn; bs < P.NMCP; bs += P.NumThreads) //// loop over populated microcells
			{
				...
				//// Code block goes through various types of treatments/interventions (vaccination/movement
				/// restrictions etc.), / assesses whether various triggers (counts) are over a certain
				/// threshold, (specified in ReadParams) / and then implements those treatments by setting
				/// various flags (i.e. .treat/ .vacc etc.) by microcell. / Further, this block assigns all
				...
				if ((Mcells[b].treat == 1) && (ts >= Mcells[b].treat_start_time))
				{
					...
					for (int i = 0; i < Mcells[b].n; i++)
					{
						...
						if ((!HOST_TO_BE_TREATED(l)) &&
								((P.TreatPropRadial == 1) || (ranf_mt(tn) < P.TreatPropRadial)))
				...
				{
					int trig_thresh =
							(P.DoPerCapitaTriggers)
									? ((int)ceil(((double)(AdUnits[adi].n * P.TreatCellIncThresh)) / P.IncThreshPop))
				...
				else
				{
					int trig_thresh =
							(P.DoPerCapitaTriggers)
									? ((int)ceil(((double)(Mcells[b].n * P.TreatCellIncThresh)) / P.IncThreshPop))
									: (int)P.TreatCellIncThresh;
				...
				if ((Mcells[b].vacc == 1) && (ts >= Mcells[b].vacc_start_time))
				{
					...
					// if(State.cumVG+P.NumThreads*Mcells[b].n<P.VaccMaxCourses) //changed to VG - commented
					// this out for now, we'll add everyone to queues and deal with the number of doses
					// available in the vaccination function
					{
						for (int i = 0; i < Mcells[b].n; i++)
						{
							int l = Mcells[b].members[i];
				...
				else if (P.DoAdminTriggers)
				{
					int trig_thresh =
							(P.DoPerCapitaTriggers)
									? ((int)ceil(((double)(AdUnits[adi].n * P.VaccCellIncThresh)) / P.IncThreshPop))
				...
				else
				{
					int trig_thresh =
							(P.DoPerCapitaTriggers)
									? ((int)ceil(((double)(Mcells[b].n * P.VaccCellIncThresh)) / P.IncThreshPop))
									: (int)P.VaccCellIncThresh;
					f2 = (Mcells[b].treat_trig >= trig_thresh);
				}
				if ((!P.DoMassVacc) && (P.VaccRadius2 > 0) && (t >= P.VaccTimeStartGeo) &&
						(Mcells[b].vacc == 0) && (f2)) // changed from VaccTimeStart to VaccTimeStarGeo
				{
					MicroCellPosition min = P.get_micro_cell_position_from_cell_index(b);
					Direction j = Right;
					int k = b;
				...
				/// threshold in order to close places for first time.
				if (P.DoGlobalTriggers)
				...
				else if (P.DoAdminTriggers)
				{
					int trig_thresh =
							(P.DoPerCapitaTriggers)
									? ((int)ceil(((double)(AdUnits[adi].n * P.PlaceCloseCellIncStopThresh)) /
															 P.IncThreshPop))
									: P.PlaceCloseCellIncStopThresh;
					f2 = (State.trigDC_adunit[adi] < trig_thresh);
				}
				else
				{
					int trig_thresh =
							(P.DoPerCapitaTriggers)
									? ((int)ceil(((double)(Mcells[b].n * P.PlaceCloseCellIncStopThresh)) /
															 P.IncThreshPop))
									: P.PlaceCloseCellIncStopThresh;
					f2 = (Mcells[b].treat_trig < trig_thresh);
				}
				if ((Mcells[b].placeclose == 2) &&
						((f2) ||
						 (ts >= Mcells[b].place_end_time))) //// if place closure has started, the places in
																								/// this microcell are closed, and either stop
																								/// threshold has been reached or place_end_time has
																								/// passed, go through block
				{
					f = 1;
				...
				{
					///// note that here f2 bool asks whether trigger has exceeded threshold in order to close
					/// places for first time.A few blocks up meaning was almost the opposite: asking whether
					/// trigger lower than stop threshold.
					...
					else if (P.DoAdminTriggers)
					{
						int trig_thresh =
								(P.DoPerCapitaTriggers)
										? ((int)ceil(((double)(AdUnits[adi].n * P.PlaceCloseCellIncThresh)) /
																 P.IncThreshPop))
					...
					{
						int trig_thresh = (P.DoPerCapitaTriggers)
																	? ((int)ceil(((double)(Mcells[b].n * P.PlaceCloseCellIncThresh)) /
																							 P.IncThreshPop))
																	: P.PlaceCloseCellIncThresh;
						f2 = (Mcells[b].treat_trig >= trig_thresh);
					}
					if (((P.PlaceCloseByAdminUnit) &&
							 (AdUnits[Mcells[b].adunit].place_close_trig < USHRT_MAX - 1) &&
							 (((double)AdUnits[Mcells[b].adunit].place_close_trig) /
										((double)AdUnits[Mcells[b].adunit].NP) >
								P.PlaceCloseAdunitPropThresh)) ||
							((!P.PlaceCloseByAdminUnit) && (f2)))
					{
						//							if(P.PlaceCloseByAdminUnit)
						// AdUnits[Mcells[b].adunit].place_close_trig=USHRT_MAX-1; // This means schools only
						// close once
						int interventionFlag; // added this as a way to help filter out when interventions start
						interventionFlag = 1;
						if ((P.DoInterventionDelaysByAdUnit) &&
								((t <= AdUnits[Mcells[b].adunit].PlaceCloseTimeStart) ||
								 (t >= (AdUnits[Mcells[b].adunit].PlaceCloseTimeStart +
												AdUnits[Mcells[b].adunit].PlaceCloseDuration))))
							interventionFlag = 0;
						...
						if ((interventionFlag == 1) && ((!P.PlaceCloseByAdminUnit) || (ad > 0)))
						{
							if ((Mcells[b].n > 0) && (Mcells[b].placeclose == 0))
							{
								...
								if (P.DoInterventionDelaysByAdUnit)
									Mcells[b].place_end_time = (unsigned short int)ceil(
											P.TimeStepsPerDay *
											(t + P.PlaceCloseDelayMean + AdUnits[Mcells[b].adunit].PlaceCloseDuration));
								else
				...
				{
					int trig_thresh = (P.DoPerCapitaTriggers)
																? ((int)ceil(((double)(AdUnits[adi].n * P.MoveRestrCellIncThresh)) /
																						 P.IncThreshPop))
																: P.MoveRestrCellIncThresh;
					f2 = (State.trigDC_adunit[adi] > trig_thresh);
				}
				else
				{
					int trig_thresh =
							(P.DoPerCapitaTriggers)
									? ((int)ceil(((double)(Mcells[b].n * P.MoveRestrCellIncThresh)) / P.IncThreshPop))
									: P.MoveRestrCellIncThresh;
					f2 = (Mcells[b].treat_trig >= trig_thresh);
				}
				...
				else if (P.DoAdminTriggers)
				{
					int trig_thresh =
							(P.DoPerCapitaTriggers)
									? ((int)ceil(((double)(AdUnits[adi].n * P.SocDistCellIncStopThresh)) /
															 P.IncThreshPop))
									: P.SocDistCellIncStopThresh;
					f2 = (State.trigDC_adunit[adi] < trig_thresh);
				}
				else
				{
					int trig_thresh = (P.DoPerCapitaTriggers)
																? ((int)ceil(((double)(Mcells[b].n * P.SocDistCellIncStopThresh)) /
																						 P.IncThreshPop))
																: P.SocDistCellIncStopThresh;
					f2 = (Mcells[b].treat_trig < trig_thresh);
				}

				//// if: policy of social distancing has started AND this microcell cell has been labelled
				/// to as undergoing social distancing, AND either trigger not reached (note definition of
				/// f2 changes in next few lines) or end time has passed.
				if ((t >= P.SocDistTimeStart) && (Mcells[b].socdist == 2) &&
						((f2) || (ts >= Mcells[b].socdist_end_time)))
				{
					f = 1;
					Mcells[b].socdist =
							P.DoSocDistOnceOnly; //// i.e. if P.DoSocDistOnceOnly set to false, socdist set to 0
																	 /// here, hence will be picked up upon some subsequent call to
																	 /// TreatSweep if required trigger passes threshold.
				...
				{
					int trig_thresh = (P.DoPerCapitaTriggers)
																? ((int)ceil(((double)(AdUnits[adi].n * P.SocDistCellIncThresh)) /
																						 P.IncThreshPop))
				...
				else
				{
					int trig_thresh =
							(P.DoPerCapitaTriggers)
									? ((int)ceil(((double)(Mcells[b].n * P.SocDistCellIncThresh)) / P.IncThreshPop))
									: P.SocDistCellIncThresh;
					f2 = (Mcells[b].treat_trig >= trig_thresh);
				}
				if ((t >= P.SocDistTimeStart) && (Mcells[b].socdist == 0) && (f2))
				{
					// some code to try and deal with intervention delays and durations by admin unit based on
					// global triggers
					int interventionFlag; // added this as a way to help filter out when interventions start
					interventionFlag = 1;
					...
					if (P.DoInterventionDelaysByAdUnit)
						if ((t <= AdUnits[Mcells[b].adunit].SocialDistanceTimeStart) ||
								(t >= (AdUnits[Mcells[b].adunit].SocialDistanceTimeStart +
											 AdUnits[Mcells[b].adunit]
													 .SocialDistanceDuration))) //// i.e. if outside window of social
																											/// distancing for this admin unit.
							interventionFlag = 0;

					if (interventionFlag == 1)
						if ((Mcells[b].n > 0) &&
								(Mcells[b].socdist ==
								 0)) //// if microcell populated and not currently undergoing social distancing
						{
							Mcells[b].socdist =
									2; //// update flag to denote that cell is undergoing social distancing
							Mcells[b].socdist_trig = 0; /// reset trigger
							//// set (admin-specific) social distancing end time.
							if (P.DoInterventionDelaysByAdUnit)
								Mcells[b].socdist_end_time = (unsigned short int)ceil(
										P.TimeStepsPerDay * (t + AdUnits[Mcells[b].adunit].SocialDistanceDuration));
							else
				...
				else if (P.DoAdminTriggers)
				{
					int trig_thresh =
							(P.DoPerCapitaTriggers)
									? ((int)ceil(((double)(AdUnits[adi].n * P.KeyWorkerProphCellIncThresh)) /
															 P.IncThreshPop))
									: P.KeyWorkerProphCellIncThresh;
					f2 = (State.trigDC_adunit[adi] > trig_thresh);
				}
				else
				{
					int trig_thresh =
							(P.DoPerCapitaTriggers)
									? ((int)ceil(((double)(Mcells[b].n * P.KeyWorkerProphCellIncThresh)) /
															 P.IncThreshPop))
									: P.KeyWorkerProphCellIncThresh;
					f2 = (Mcells[b].treat_trig >= trig_thresh);
				}
				if ((P.DoPlaces) && (t >= P.KeyWorkerProphTimeStart) && (Mcells[b].keyworkerproph == 0) &&
						(f2))
				{
					MicroCellPosition min = P.get_micro_cell_position_from_cell_index(b);
					Direction j = Right;
