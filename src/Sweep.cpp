...
void UpdateQueue(int j, int i, int k, int contact, int infector, unsigned short contact_time);
void EndDigital(int contact);
unsigned short GetStartTime(int infector, int contact, unsigned short contact_time);
unsigned short GetEndTime(int infector, unsigned short& dct_start_time, int contact, unsigned short contact_time);
						...
						if (dct_start_time == ts)
						{
							...
							if (Hosts[contact].digitalContactTraced == 0)
							{
								...
								if (AdUnits[i].ndct < AdUnits[i].n) //AdUnits[i].n is length of queue
								{
									...
									UpdateQueue(j, i, k, contact, infector, contact_time);
								}
								else
								{
