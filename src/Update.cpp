...
void RecordEvent(
		double, int, int, int,
		int); // added int as argument to InfectSweep to record run number: ggilani - 15/10/14
...
{
	// This transfers a person straight from susceptible to immune. Used to start a run with a
	...
	if (a->inf == InfStat_Susceptible)
	{
		...
		if (Cells[c].I > 0)
		{
			Cells[c].susceptible[Cells[c].S + Cells[c].L] =
					Cells[c].susceptible[Cells[c].S + Cells[c].L + Cells[c].I];
...
void DoInfect(int ai, double t, int tn,
							int run) // Change person from susceptible to latently infected.  added int as
											 // argument to DoInfect to record run number: ggilani - 15/10/14
{
	///// This updates a number of things concerning person ai (and their contacts/infectors/places
	/// etc.) at time t in thread tn for this run.
	int i;
	unsigned short int ts; //// time step
	double q, x, y; //// q radius squared, x and y coords. q later changed to be quantile of inverse
									/// CDF to choose latent period.
	Person *a;

	a = Hosts + ai; //// pointer arithmetic. a = pointer to person. ai = int person index.

	if (a->inf == InfStat_Susceptible) //// Only change anything if person a/ai uninfected at start of
																		 /// this function.
	{
		...
		if (q > StateT[tn].maxRad2)
			StateT[tn].maxRad2 = q; //// update maximum radius squared from seeding infection
		{
			...
			if (a->listpos < Cells[a->pcell].S)
			{
				...
				a->listpos = Cells[a->pcell].S; //// person a's position with cell.members now equal to
																				/// number of susceptibles in cell.
		...
		if (P.DoLatent)
		{
			...
			q -= ((double)i);
			a->latent_time = (unsigned short int)floor(
					0.5 +
					(t - P.LatentPeriod * log(q * P.latent_icdf[i + 1] + (1.0 - q) * P.latent_icdf[i])) *
							P.TimeStepsPerDay);
		}
		else
		...
		// if (P.DoLatent)	a->latent_time = a->infection_time + ChooseFromICDF(P.latent_icdf,
		// P.LatentPeriod, tn); else			a->latent_time = (unsigned short int) (t * P.TimeStepsPerDay);
		...
		if (P.OutputBitmap)
		{
			if ((P.OutputBitmapDetected == 0) ||
					((P.OutputBitmapDetected == 1) && (Hosts[ai].detected == 1)))
			{
				Vector2<int> pixel((Households[a->hh].loc * P.scale) - P.bmin);
				if (P.b.contains(pixel))
				{
...
#pragma omp atomic
		...
		if (P.DoRecordInfEvents)
		{
			RecordEvent(
					t, ai, run, 0,
					tn); // added int as argument to RecordEvent to record run number: ggilani - 15/10/14
		}
		if ((t > 0) && (P.DoOneGen))
		{
...
void RecordEvent(
		double t, int ai, int run, int type,
		int tn) // added int as argument to RecordEvent to record run number: ggilani - 15/10/14
{
...
void DoRecoveringFromCritical(int ai, int tn)
{
	...
	//// DoRecoveringFromCritical assigns people to intermediate state "recovering from critical
	/// condition" (and bookkeeps accordingly).
	if (P.DoSeverity) //// shouldn't need this but best be careful.
	{
		Person *a = Hosts + ai;
		if (a->Severity_Current == Severity_Critical &&
				(!a->to_die)) //// second condition should be unnecessary but leave in for now.
		{
...
void DoDeath_FromCriticalorSARIorILI(int ai, int tn)
{
	...
	if (P.DoSeverity)
	{
		if (a->Severity_Current == Severity_Critical)
		{
			...
			/// Don't move this outside of this if statement, even though it looks like it can be moved
			/// safely. It can't.
...
void DoRecover_FromSeverity(int ai, int tn)
{
	...
	if (P.DoSeverity)
		if (a->inf == InfStat_InfectiousAsymptomaticNotCase ||
				a->inf == InfStat_Case) ///// i.e same condition in DoRecover (make sure you don't recover
																/// people twice).
		{
...
void DoIncub(int ai, unsigned short int ts, int tn, int run)
{
	...
	if (a->inf == InfStat_Latent)
	{
		...
		if (P.InfectiousnessSD > 0)
			a->infectiousness *= (float)gen_gamma_mt(1 / (P.InfectiousnessSD * P.InfectiousnessSD),
																							 1 / (P.InfectiousnessSD * P.InfectiousnessSD), tn);
		q = P.ProportionSymptomatic[age] * (HOST_TREATED(ai) ? (1 - P.TreatSympDrop) : 1) *
		...
		if (!P.DoSeverity ||
				a->inf == InfStat_InfectiousAsymptomaticNotCase) //// if not doing severity or if person
																												 /// asymptomatic.
		{
			if (P.DoInfectiousnessProfile)
				a->recovery_or_death_time =
						a->latent_time + (unsigned short int)(P.InfectiousPeriod * P.TimeStepsPerDay);
			else
				a->recovery_or_death_time =
						a->latent_time + ChooseFromICDF(P.infectious_icdf, P.InfectiousPeriod, tn);
		}
		else
		{
			int CaseTime = a->latent_time +
										 ((int)(P.LatentToSymptDelay /
														P.TimeStep)); //// base severity times on CaseTime, not latent time.
																					/// Otherwise there are edge cases where recovery time is
																					/// zero days after latent_time and therefore before
																					/// DoCase called in IncubRecoverySweep (i.e. people can
																					/// recover before they've become a case!).

			//// choose final disease severity (either mild, ILI, SARI, Critical, not asymptomatic as
			/// covered above) by age
			...
			if (a->Severity_Final == Severity_Mild)
				a->recovery_or_death_time =
						CaseTime + ChooseFromICDF(P.MildToRecovery_icdf, P.Mean_MildToRecovery[age], tn);
			else if (a->Severity_Final == Severity_Critical)
			{
				a->SARI_time = CaseTime + ChooseFromICDF(P.ILIToSARI_icdf, P.Mean_ILIToSARI[age], tn);
				a->Critical_time =
						a->SARI_time + ChooseFromICDF(P.SARIToCritical_icdf, P.Mean_SARIToCritical[age], tn);
				if (a->to_die)
					a->recovery_or_death_time =
							a->Critical_time +
							ChooseFromICDF(P.CriticalToDeath_icdf, P.Mean_CriticalToDeath[age], tn);
				else
				{
					a->RecoveringFromCritical_time =
							a->Critical_time +
							ChooseFromICDF(P.CriticalToCritRecov_icdf, P.Mean_CriticalToCritRecov[age], tn);
					a->recovery_or_death_time =
							a->RecoveringFromCritical_time +
							ChooseFromICDF(P.CritRecovToRecov_icdf, P.Mean_CritRecovToRecov[age], tn);
				}
			}
			else if (a->Severity_Final == Severity_SARI)
			{
				...
				if (a->to_die)
					a->recovery_or_death_time =
							a->SARI_time + ChooseFromICDF(P.SARIToDeath_icdf, P.Mean_SARIToDeath[age], tn);
				else
					a->recovery_or_death_time =
							a->SARI_time + ChooseFromICDF(P.SARIToRecovery_icdf, P.Mean_SARIToRecovery[age], tn);
			}
			else /*i.e. if Severity_Final == Severity_ILI*/
			{
				if (a->to_die)
					a->recovery_or_death_time =
							CaseTime + ChooseFromICDF(P.ILIToDeath_icdf, P.Mean_ILIToDeath[age], tn);
				else
					a->recovery_or_death_time =
							CaseTime + ChooseFromICDF(P.ILIToRecovery_icdf, P.Mean_ILIToRecovery[age], tn);
			}
		}

		if ((a->inf == InfStat_InfectiousAlmostSymptomatic) &&
				((P.ControlPropCasesId == 1) || (ranf_mt(tn) < P.ControlPropCasesId)))
		{
			Hosts[ai].detected = 1;
			Hosts[ai].detected_time = ts + (unsigned short int)(P.LatentToSymptDelay * P.TimeStepsPerDay);

			if ((P.DoDigitalContactTracing) &&
					(Hosts[ai].detected_time >= (unsigned short int)(AdUnits[Mcells[Hosts[ai].mcell].adunit]
																															 .DigitalContactTracingTimeStart *
																													 P.TimeStepsPerDay)) &&
					(Hosts[ai].detected_time < (unsigned short int)((AdUnits[Mcells[Hosts[ai].mcell].adunit]
																															 .DigitalContactTracingTimeStart +
																													 P.DigitalContactTracingPolicyDuration) *
																													P.TimeStepsPerDay)) &&
					(Hosts[ai].digitalContactTracingUser))
			{
				// set dct_trigger_time for index case
				if (P.DoDigitalContactTracing) // set dct_trigger_time for index case
					if (Hosts[ai].dct_trigger_time ==
							(USHRT_MAX - 1)) // if this hasn't been set in DigitalContactTracingSweep due to
															 // detection of contact of contacts, set it here
						Hosts[ai].dct_trigger_time =
								Hosts[ai].detected_time +
								(unsigned short int)(P.DelayFromIndexCaseDetectionToDCTIsolation *
																		 P.TimeStepsPerDay);
			}
		}
		...
		{
			Cells[a->pcell].susceptible[a->listpos] =
					Cells[a->pcell].latent[Cells[a->pcell].L]; //// reset pointers.
			Hosts[Cells[a->pcell].susceptible[a->listpos]].listpos = a->listpos;
			a->listpos = Cells[a->pcell].S +
									 Cells[a->pcell].L; //// change person a's listpos, which will now refer to their
																			/// position among infectious people, not latent.
			Cells[a->pcell].infected[0] =
					ai; //// this person is now first infectious person in the array. Pointer was moved back
							/// one so now that memory address refers to person ai. Alternative would be to move
							/// everyone back one which would take longer.
		}
	}
}
...
void DoDetectedCase(int ai, double t, unsigned short int ts, int tn)
{
	...
	//// and therefore changes lots of quantities (e.g. quar_comply and isolation_start_time)
	/// associated with model macros e.g. HOST_ABSENT / HOST_ISOLATED

	int j, k, f, j1, j2, ad; // m, h, ad;
	Person *a = Hosts + ai;

	//// Increment triggers (Based on numbers of detected cases) for interventions. Used in TreatSweep
	/// function when not doing Global or Admin triggers. And not when doing ICU triggers.
	if (Mcells[a->mcell].treat_trig < USHRT_MAX - 1)
		Mcells[a->mcell].treat_trig++;
	if (Mcells[a->mcell].vacc_trig < USHRT_MAX - 1)
		Mcells[a->mcell].vacc_trig++;
	if (Mcells[a->mcell].move_trig < USHRT_MAX - 1)
		Mcells[a->mcell].move_trig++;
	if (Mcells[a->mcell].socdist_trig < USHRT_MAX - 1)
		Mcells[a->mcell].socdist_trig++;
	if (Mcells[a->mcell].keyworkerproph_trig < USHRT_MAX - 1)
		Mcells[a->mcell].keyworkerproph_trig++;

	if (!P.AbsenteeismPlaceClosure)
	{
		if ((P.PlaceCloseRoundHousehold) && (Mcells[a->mcell].place_trig < USHRT_MAX - 1))
			Mcells[a->mcell].place_trig++;
		if ((t >= P.PlaceCloseTimeStart) && (!P.DoAdminTriggers) &&
				(!((P.DoGlobalTriggers) && (P.PlaceCloseCellIncThresh < 1000000000))))
		...
		if ((P.TreatPropCases == 1) || (ranf_mt(tn) < P.TreatPropCases))
		{
			...
			if (P.DoHouseholds)
			{
				if ((t < P.TreatTimeStart + P.TreatHouseholdsDuration) &&
						((P.TreatPropCaseHouseholds == 1) || (ranf_mt(tn) < P.TreatPropCaseHouseholds)))
				{
					j1 = Households[Hosts[ai].hh].FirstPerson;
					j2 = j1 + Households[Hosts[ai].hh].nh;
					for (j = j1; j < j2; j++)
						if (!HOST_TO_BE_TREATED(j))
							DoProph(j, ts, tn);
				}
			}
			if (P.DoPlaces)
...
#pragma omp critical(starttreat)
								if (!Places[j][a->PlaceLinks[j]].treat)
									f = Places[j][a->PlaceLinks[j]].treat = 1;
								if (f)
								{
									if ((P.TreatPlaceProbCaseId[j] == 1) || (ranf_mt(tn) < P.TreatPlaceProbCaseId[j]))
										StateT[tn].p_queue[j][StateT[tn].np_queue[j]++] = a->PlaceLinks[j];
...
#pragma omp critical(state_cumV)
			...
			if (cumV_OK && (t < P.VaccTimeStart + P.VaccHouseholdsDuration) &&
					((P.VaccPropCaseHouseholds == 1) || (ranf_mt(tn) < P.VaccPropCaseHouseholds)))
			{
				j1 = Households[Hosts[ai].hh].FirstPerson;
				j2 = j1 + Households[Hosts[ai].hh].nh;
				for (j = j1; j < j2; j++)
		...
		//// Giant compound if statement. If doing delays by admin unit, then window of HQuarantine
		/// dependent on admin unit-specific duration. This if statement ensures that this timepoint
		/// within window, regardless of how window defined.
		if ((P.DoInterventionDelaysByAdUnit &&
				 (t >= AdUnits[Mcells[a->mcell].adunit].HQuarantineTimeStart &&
					(t < AdUnits[Mcells[a->mcell].adunit].HQuarantineTimeStart +
									 AdUnits[Mcells[a->mcell].adunit].HQuarantineDuration))) ||
				(t >= AdUnits[Mcells[a->mcell].adunit].HQuarantineTimeStart &&
				 (t < AdUnits[Mcells[a->mcell].adunit].HQuarantineTimeStart + P.HQuarantinePolicyDuration)))
		{
			j1 = Households[Hosts[ai].hh].FirstPerson;
			j2 = j1 + Households[Hosts[ai].hh].nh;
			if ((!HOST_TO_BE_QUARANTINED(j1)) || (P.DoHQretrigger))
			{
				Hosts[j1].quar_start_time =
						ts + ((unsigned short int)(P.TimeStepsPerDay * P.HQuarantineDelay));
				k = (ranf_mt(tn) < P.HQuarantinePropHouseCompliant)
								? 1
								: 0; //// Is household compliant? True or false
				if (k)
					StateT[tn].cumHQ++; ////  if compliant, increment cumulative numbers of households under
															/// quarantine.
				//// if household not compliant then neither is first person. Otheswise ask whether first
				/// person is compliant?
				...
				for (j = j1; j < j2; j++)
				{
					...
					Hosts[j].quar_comply =
							((k == 0) ? 0 : ((ranf_mt(tn) < P.HQuarantinePropIndivCompliant) ? 1 : 0));
					if ((Hosts[j].quar_comply) && (!HOST_ABSENT(j)))
					{
						if (HOST_AGE_YEAR(j) >= P.CaseAbsentChildAgeCutoff)
						{
							if (Hosts[j].PlaceLinks[P.PlaceTypeNoAirNum - 1] >= 0)
								StateT[tn].cumAH++;
						}
						else
							StateT[tn].cumACS++;
	...
	//// Giant compound if statement. If doing delays by admin unit, then window of case isolation
	/// dependent on admin unit-specific duration. This if statement ensures that this timepoint
	/// within window, regardless of how window defined.
	if ((P.DoInterventionDelaysByAdUnit &&
			 (t >= AdUnits[Mcells[a->mcell].adunit].CaseIsolationTimeStart &&
				(t < AdUnits[Mcells[a->mcell].adunit].CaseIsolationTimeStart +
								 AdUnits[Mcells[a->mcell].adunit].CaseIsolationPolicyDuration))) ||
			(t >= AdUnits[Mcells[a->mcell].adunit].CaseIsolationTimeStart &&
			 (t <
				AdUnits[Mcells[a->mcell].adunit].CaseIsolationTimeStart + P.CaseIsolationPolicyDuration)))
		if ((P.CaseIsolationProp == 1) || (ranf_mt(tn) < P.CaseIsolationProp))
		{
			Hosts[ai].isolation_start_time = ts; //// set isolation start time.
			if (HOST_ABSENT(ai))
			{
				if (a->absent_stop_time <
						ts + P.usCaseAbsenteeismDelay +
								P.usCaseIsolationDuration) //// ensure that absent_stop_time is at least now +
																					 /// CaseIsolationDuraton
					a->absent_stop_time = ts + P.usCaseAbsenteeismDelay + P.usCaseIsolationDuration;
			}
			else if (P.DoRealSymptWithdrawal) /* This calculates adult absenteeism from work due to care
																					 of isolated children.  */
			{
				...
				if (P.DoPlaces)
				{
					if ((!HOST_QUARANTINED(ai)) && (Hosts[ai].PlaceLinks[P.PlaceTypeNoAirNum - 1] >= 0) &&
							(HOST_AGE_YEAR(ai) >= P.CaseAbsentChildAgeCutoff))
						StateT[tn].cumAC++;
				}
				if ((P.DoHouseholds) && (P.DoPlaces) &&
						(HOST_AGE_YEAR(ai) < P.CaseAbsentChildAgeCutoff)) //// if host is a child who requires
																															/// adult to stay at home.
				{
					if (!HOST_QUARANTINED(ai))
						StateT[tn].cumACS++;
					if (Hosts[ai].ProbCare <
							P.CaseAbsentChildPropAdultCarers) //// if adult needs to stay at home (i.e. if
																								/// Proportion of children at home for whom one
																								/// adult also stays at home = 1 or coinflip
																								/// satisfied.)
					{
						j1 = Households[Hosts[ai].hh].FirstPerson;
						j2 = j1 + Households[Hosts[ai].hh].nh;
						...
						//// in loop below, f true if any household member a) alive AND b) not a child AND c)
						/// has no links to workplace (or is absent from work or quarantined).
						for (j = j1; (j < j2) && (!f); j++)
							f = ((abs(Hosts[j].inf) != InfStat_Dead) &&
									 (HOST_AGE_YEAR(j) >= P.CaseAbsentChildAgeCutoff) &&
									 ((Hosts[j].PlaceLinks[P.PlaceTypeNoAirNum - 1] < 0) || (HOST_ABSENT(j)) ||
										(HOST_QUARANTINED(j))));

						//// so !f true if any household member EITHER: a) dead; b) a child; c) has a link to an
						/// office and not currently absent or quarantined.
						if (!f) //// so if either a) a household member is dead; b) a household member is a
										/// child requiring adult to stay home; c) a household member has links to
										/// office.
						{
							for (j = j1; (j < j2) & (!f); j++) /// loop again, checking whether household members
																								 /// not children needing supervision and are alive.
								if ((HOST_AGE_YEAR(j) >= P.CaseAbsentChildAgeCutoff) &&
										(abs(Hosts[j].inf) != InfStat_Dead))
								{
									k = j;
									f = 1;
								}
							if (f) //// so finally, if at least one member of household is alive and does not need
										 /// supervision by an adult, amend absent start and stop times
							{
	...
	// add contacts to digital contact tracing, but only if considering contact tracing, we are within
	// the window of the policy and the detected case is a user
	if ((P.DoDigitalContactTracing) &&
			(t >= AdUnits[Mcells[Hosts[ai].mcell].adunit].DigitalContactTracingTimeStart) &&
			(t < AdUnits[Mcells[Hosts[ai].mcell].adunit].DigitalContactTracingTimeStart +
							 P.DigitalContactTracingPolicyDuration) &&
			(Hosts[ai].digitalContactTracingUser))
	{
		...
		if ((P.DCTIsolateIndexCases) &&
				(Hosts[ai].index_case_dct ==
				 0)) //(Hosts[ai].digitalContactTraced == 0)&& - currently removed this condition as it
						 // would mean that someone already under isolation wouldn't have their isolation
						 // extended
		{
			...
			{
				// if we are isolating an index case, we set their infector as -1 in order to get the
				// timings consistent.
			...
			else
			{
				fprintf(stderr, "No more space in queue! AdUnit: %i, ndct=%i, max queue length: %i\n", ad,
								AdUnits[j].ndct, AdUnits[ad].n);
				fprintf(stderr, "Error!\n");
			}
		}
		// currently commenting this out as household members will likely be picked up by household
		// quarantine. can add back in if needed, but would need to re-add a couple more local
		// variables.

		// if(P.IncludeHouseholdDigitalContactTracing)
		//{
		//	//Then we want to find all their household and place group contacts to add to the contact
		// tracing queue
		//	//Start with household contacts
		//	j1 = Households[Hosts[ai].hh].FirstPerson; j2 = j1 + Households[Hosts[ai].hh].nh;
		//	for (j = j1; j < j2; j++)
		//	{
		//		//if host is dead or the detected case, no need to add them to the list. They also need to
		// be a user themselves 		if ((abs(Hosts[j].inf) != 5) && (j != ai) &&
		//(Hosts[j].digitalContactTracingUser) && (ranf_mt(tn)<P.ProportionDigitalContactsIsolate))
		//		{
		...
		//				//if host is dead or the detected case, no need to add them to the list. They also
		// need to be a user themselves 				if ((abs(Hosts[h].inf) != 5) && (h != ai) &&
		//(Hosts[h].digitalContactTracingUser))// && (ranf_mt(tn)<P.ProportionDigitalContactsIsolate))
		//				{
		//					ad = Mcells[Hosts[h].mcell].adunit;
		//					if ((StateT[tn].ndct_queue[ad] < P.InfQueuePeakLength))
		//					{
		...
		//						StateT[tn].contacts[ad][StateT[tn].ncontacts[ad]++] = ai; //keep a record of who
		// the detected case was
		//					}
		//					else
		//					{
		//						fprintf(stderr, "No space left in queue! Thread: %i, AdUnit: %i\n", tn, ad);
...
void DoCase(int ai, double t, unsigned short int ts,
						int tn) //// makes an infectious (but asymptomatic) person symptomatic. Called in
										/// IncubRecoverySweep (and DoInfect if P.DoOneGen)
{
	int j, k, f, j1, j2;
	Person *a;
	int age;
	...
	if (age >= NUM_AGE_GROUPS)
		age = NUM_AGE_GROUPS - 1;
	a = Hosts + ai;
	if (a->inf == InfStat_InfectiousAlmostSymptomatic) //// if person latent/asymptomatically
																										 /// infected, but infectious
	{
		...
		else if ((P.DoRealSymptWithdrawal) && (P.DoPlaces))
		{
			...
			for (j = 0; j < P.PlaceTypeNum; j++)
				if ((a->PlaceLinks[j] >= 0) && (j != P.HotelPlaceType) && (!HOST_ABSENT(ai)) &&
						(P.SymptPlaceTypeWithdrawalProp[j] > 0))
				{
					if ((P.SymptPlaceTypeWithdrawalProp[j] == 1) ||
							(ranf_mt(tn) < P.SymptPlaceTypeWithdrawalProp[j]))
					{
						...
						if ((!HOST_QUARANTINED(ai)) && (Hosts[ai].PlaceLinks[P.PlaceTypeNoAirNum - 1] >= 0) &&
								(HOST_AGE_YEAR(ai) >= P.CaseAbsentChildAgeCutoff))
							StateT[tn].cumAC++;
						/* This calculates adult absenteeism from work due to care of sick children. Note,
						children not at school not counted (really this should be fixed in population setup by
						having adult at home all the time for such kids. */
						if ((P.DoHouseholds) && (HOST_AGE_YEAR(ai) < P.CaseAbsentChildAgeCutoff))
						{
							...
							if (Hosts[ai].ProbCare < P.CaseAbsentChildPropAdultCarers)
							{
								...
								for (int j3 = j1; (j3 < j2) && (!f); j3++)
									f = ((abs(Hosts[j3].inf) != InfStat_Dead) &&
											 (HOST_AGE_YEAR(j3) >= P.CaseAbsentChildAgeCutoff) &&
											 ((Hosts[j3].PlaceLinks[P.PlaceTypeNoAirNum - 1] < 0) || (HOST_ABSENT(j3)) ||
												(HOST_QUARANTINED(j3))));
								if (!f)
								{
									for (int j3 = j1; (j3 < j2) && (!f); j3++)
										if ((HOST_AGE_YEAR(j3) >= P.CaseAbsentChildAgeCutoff) &&
												(abs(Hosts[j3].inf) != InfStat_Dead))
										{
											k = j3;
											f = 1;
										}
									if (f)
									{
										...
										Hosts[k].absent_stop_time =
												ts + P.usCaseIsolationDelay + P.usCaseIsolationDuration;
										StateT[tn].cumAA++;
									}
								}
							}
		...
		StateT[tn].cumCa[age]++;
		StateT[tn]
				.cumC_country[Mcells[Hosts[ai].mcell].country]++; // add to cumulative count of cases in
																													// that country: ggilani - 12/11/14
		...
		if (P.DoSeverity)
		{
			if (a->Severity_Final == Severity_Mild)
				...
				DoILI(ai, tn); //// symptomatic cases either mild or ILI at symptom onset. SARI and Critical
											 /// cases still onset with ILI.
		}
		if (P.DoAdUnits)
			StateT[tn].cumC_adunit[Mcells[a->mcell].adunit]++;
	}
}

void DoFalseCase(int ai, double t, unsigned short int ts, int tn)
{
	/* Arguably adult absenteeism to take care of sick kids could be included here, but then output
	 * absenteeism would not be 'excess' absenteeism */
	if ((P.ControlPropCasesId == 1) || (ranf_mt(tn) < P.ControlPropCasesId))
	{
		if ((!P.DoEarlyCaseDiagnosis) || (State.cumDC >= P.CaseOrDeathThresholdBeforeAlert))
			StateT[tn].cumDC++;
		DoDetectedCase(ai, t, ts, tn);
	}
	StateT[tn].cumFC++;
...
#endif
		{
			Hosts[ai].treat_start_time =
					ts + ((unsigned short int)(P.TimeStepsPerDay * P.TreatDelayMean));
			Hosts[ai].treat_stop_time =
					ts +
					((unsigned short int)(P.TimeStepsPerDay * (P.TreatDelayMean + P.TreatCaseCourseLength)));
			StateT[tn].cumT++;
			if ((abs(Hosts[ai].inf) > InfStat_Susceptible) && (Hosts[ai].inf != InfStat_Dead_WasAsymp))
				Cells[Hosts[ai].pcell].cumTC++;
			StateT[tn].cumT_keyworker[Hosts[ai].keyworker]++;
			if ((++Hosts[ai].num_treats) < 2)
				StateT[tn].cumUT++;
			Cells[Hosts[ai].pcell].tot_treat++;
			if (P.DoAdUnits)
				StateT[tn].cumT_adunit[Mcells[Hosts[ai].mcell].adunit]++;
			if (P.OutputBitmap)
			{
...
{
	//// almost identical to DoProphNoDelay, except unsurprisingly this function includes delay
	/// between timestep and start of treatment. Also increments StateT[tn].cumT_keyworker by 1 every
	/// time.
...
void DoProphNoDelay(int ai, unsigned short int ts, int tn, int nc)
{
	if (State.cumT < P.TreatMaxCourses)
	{
		...
		Hosts[ai].treat_stop_time =
				ts + ((unsigned short int)(P.TimeStepsPerDay * P.TreatProphCourseLength * nc));
		StateT[tn].cumT += nc;
		StateT[tn].cumT_keyworker[Hosts[ai].keyworker] += nc;
		if ((++Hosts[ai].num_treats) < 2)
			StateT[tn].cumUT++;
		if (P.DoAdUnits)
			StateT[tn].cumT_adunit[Mcells[Hosts[ai].mcell].adunit] += nc;
#pragma omp critical(tot_treat)
		Cells[Hosts[ai].pcell].tot_treat++;
		if (P.OutputBitmap)
		{
...
{
	//// DoPlaceClose function called in TreatSweep (with arg DoAnyway = 1) and DoDetectedCase (with
	/// arg DoAnyway = 0). / Basic pupose of this function is to change Places[i][j].close_start_time
	/// and Places[i][j].close_end_time, so that macro PLACE_CLOSED will return true. / This will then
	/// scale peoples household, place, and spatial infectiousness and susceptibilities in function
	/// InfectSweep (but not in functions ini CalcInfSusc.cpp)
	...
	*/
	...
	if (P.DoInterventionDelaysByAdUnit)
	{
		...
		t_stop = ts + ((unsigned short int)(P.TimeStepsPerDay *
																				(P.PlaceCloseDelayMean + AdUnits[k].PlaceCloseDuration)));
	}
	else
	{
		t_stop =
...
#pragma omp critical(closeplace)
	{
		...
		//// close_end_time initialized to zero in InitModel (so will pass this check on at least first
		...
		if (Places[i][j].close_end_time < t_stop)
		{
			if ((!DoAnyway) && (Places[i][j].control_trig < USHRT_MAX - 2))
			{
				...
				if (P.AbsenteeismPlaceClosure)
				{
					...
					if (t_new >= t_old + P.MaxAbsentTime)
						for (l = 0; l < P.MaxAbsentTime; l++)
							Places[i][j].Absent[l] = 0;
					else
						for (l = t_old; l < t_new; l++)
							Places[i][j].Absent[l % P.MaxAbsentTime] = 0;
					for (l = t_new; l < t_new + P.usCaseAbsenteeismDuration / P.TimeStepsPerDay; l++)
						Places[i][j].Absent[l % P.MaxAbsentTime]++;
					...
					{
						// fprintf(stderr,"** %i %i %i %i %lg ## ",i,j,(int) Places[i][j].control_trig, (int)
						// Places[i][j].n,P.PlaceCloseCasePropThresh);
						k = Mcells[Places[i][j].mcell].adunit;
						if (AdUnits[k].place_close_trig < USHRT_MAX - 1)
							AdUnits[k].place_close_trig++;
					}
				}
				else
				{
					...
					if ((P.PlaceCloseByAdminUnit) && (P.PlaceCloseAdunitPlaceTypes[i] > 0) &&
							(((double)Places[i][j].control_trig) / ((double)Places[i][j].n) >
							 P.PlaceCloseCasePropThresh))
					{
						// fprintf(stderr,"** %i %i %i %i %lg ## ",i,j,(int) Places[i][j].control_trig, (int)
						// Places[i][j].n,P.PlaceCloseCasePropThresh);
						k = Mcells[Places[i][j].mcell].adunit;
			...
			if (Places[i][j].control_trig <
					USHRT_MAX - 1) //// control_trig initialized to zero so this check will pass at least once
			{
				...
				if (((!P.PlaceCloseByAdminUnit) && (k)) || (DoAnyway))
				{
					if (P.DoPlaceCloseOnceOnly)
						Places[i][j].control_trig =
								USHRT_MAX -
								1; //// Places only close once, and so this code block would not be entered again.
					else
						Places[i][j].control_trig = 0; //// otherwise reset the trigger.

					//// set close_start_time and close_end_time

					if (Places[i][j].ProbClose >=
							P.PlaceCloseEffect[i]) //// if proportion of places of type i remaining open is 0 or
																		 /// if place is closed with prob 1 - PlaceCloseEffect[i]...
					{
						if (Places[i][j].close_start_time > t_start)
							Places[i][j].close_start_time = t_start;
						Places[i][j].close_end_time = t_stop;
						f2 = 1; /// /set flag to true so next part of function used.
					}
					else
						Places[i][j].close_start_time = Places[i][j].close_end_time =
								t_stop; //// ... otherwise set start and end of closure to be the same, which will
												/// cause macro PLACE_CLOSED to always return false.
				}
			}
		}
	}
	...
	if (f2)
	{
		if (P.DoRealSymptWithdrawal)
			for (k = 0; k < Places[i][j].n; k++) //// loop over all people in place.
			{
				...
				if (((P.PlaceClosePropAttending[i] == 0) ||
						 (Hosts[ai].ProbAbsent >= P.PlaceClosePropAttending[i])))
				{
					if ((!HOST_ABSENT(ai)) && (!HOST_QUARANTINED(ai)) &&
							(HOST_AGE_YEAR(ai) < P.CaseAbsentChildAgeCutoff)) //// if person is a child and
																																/// neither absent nor quarantined
					{
						StateT[tn].cumAPCS++;
						if (Hosts[ai].ProbCare <
								P.CaseAbsentChildPropAdultCarers) //// if child needs adult supervision
						{
							j1 = Households[Hosts[ai].hh].FirstPerson;
							j2 = j1 + Households[Hosts[ai].hh].nh;
							if ((j1 < 0) || (j2 > P.PopSize))
								fprintf(stderr, "++ %i %i %i (%i %i %i)##  ", ai, j1, j2, i, j, k);
							f = 0;

							//// in loop below, f true if any household member a) alive AND b) not a child AND c)
							/// has no links to workplace (or is absent from work or quarantined).
							for (l = j1; (l < j2) && (!f); l++)
								f = ((abs(Hosts[l].inf) != InfStat_Dead) &&
										 (HOST_AGE_YEAR(l) >= P.CaseAbsentChildAgeCutoff) &&
										 ((Hosts[l].PlaceLinks[P.PlaceTypeNoAirNum - 1] < 0) || (HOST_QUARANTINED(l))));
							if (!f) //// so !f true if there's no living adult household member who is not
											/// quarantined already or isn't a home-worker.
							{
								for (l = j1; (l < j2) && (!f);
										 l++) //// loop over all household members of child this place: find the adults
													/// and ensure they're not dead...
									if ((HOST_AGE_YEAR(l) >= P.CaseAbsentChildAgeCutoff) &&
											(abs(Hosts[l].inf) != InfStat_Dead))
									{
										if (Hosts[l].absent_start_time > t_start)
											Hosts[l].absent_start_time = t_start;
										if (Hosts[l].absent_stop_time < t_stop)
					...
					{
						///// finally amend absent start and stop times if they contradict place start and stop
						/// times.
						if (Hosts[ai].absent_start_time > t_start)
							Hosts[ai].absent_start_time = t_start;
						if (Hosts[ai].absent_stop_time < t_stop)
							Hosts[ai].absent_stop_time = t_stop;
					}
					if ((HOST_AGE_YEAR(ai) >= P.CaseAbsentChildAgeCutoff) &&
							(Hosts[ai].PlaceLinks[P.PlaceTypeNoAirNum - 1] >= 0))
						StateT[tn].cumAPC++;
				}
			}
	}
							...
							P.CaseAbsentChildPropAdultCarers) //// if child needs adult supervision
					{
						if ((HOST_AGE_YEAR(ai) < P.CaseAbsentChildAgeCutoff) && (!HOST_QUARANTINED(ai)))
						{
							...
							if (!f)
							{
								for (l = j1; (l < j2) && (!f); l++)
									if ((HOST_AGE_YEAR(l) >= P.CaseAbsentChildAgeCutoff) &&
											(abs(Hosts[l].inf) != InfStat_Dead) && (HOST_ABSENT(l)))
									{
										if (Hosts[l].absent_stop_time == Places[i][j].close_end_time)
											Hosts[l].absent_stop_time = ts;
									}
							}
						}
					}
...
void DoVacc(int ai, unsigned short int ts)
{
	...
	if ((HOST_TO_BE_VACCED(ai)) || (Hosts[ai].inf < InfStat_InfectiousAlmostSymptomatic) ||
			(Hosts[ai].inf >= InfStat_Dead_WasAsymp))
		return;
#pragma omp critical(state_cumV)
...
Severity ChooseFinalDiseaseSeverity(int AgeGroup, int tn)
{
	...
	x = ranf_mt(tn);
	if (x < P.Prop_ILI_ByAge[AgeGroup])
		DiseaseSeverity = Severity_ILI;
	else if (x < P.Prop_ILI_ByAge[AgeGroup] + P.Prop_SARI_ByAge[AgeGroup])
		DiseaseSeverity = Severity_SARI;
	else if (x < P.Prop_ILI_ByAge[AgeGroup] + P.Prop_SARI_ByAge[AgeGroup] +
									 P.Prop_Critical_ByAge[AgeGroup])
		DiseaseSeverity = Severity_Critical;
	else
		DiseaseSeverity = Severity_Mild;
...
unsigned short int ChooseFromICDF(double *ICDF, double Mean, int tn)
{
	...
	ti = -Mean *
			 log(q * ICDF[i + 1] +
					 (1.0 - q) * ICDF[i]); //// weighted average (sort of) between quartile values from
																 /// CDF_RES. logged as it was previously exponentiated in
																 /// ReadParams. Minus as exp(-cdf) was done in ReadParaams. Sort of
