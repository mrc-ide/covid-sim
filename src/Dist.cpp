...
double dist2_mm(Microcell* a, Microcell* b)
{
	...
	m = (int)(b - Mcells);
	if (P.DoUTM_coords)
		return dist2UTM(P.in_microcells_.width_ * fabs((double)(l / P.nmch)), P.in_microcells_.height_ * fabs((double)(l % P.nmch)),
			P.in_microcells_.width_ * fabs((double)(m / P.nmch)), P.in_microcells_.height_ * fabs((double)(m % P.nmch)));
	else
	{
		x = P.in_microcells_.width_ * fabs((double)(l / P.nmch - m / P.nmch));
		y = P.in_microcells_.height_ * fabs((double)(l % P.nmch - m % P.nmch));
		if (P.DoPeriodicBoundaries)
		{
			if (x > P.in_degrees_.width_ * 0.5) x = P.in_degrees_.width_ - x;
			if (y > P.in_degrees_.height_ * 0.5) y = P.in_degrees_.height_ - y;
