...
double CalcHouseInf(int j, unsigned short int TimeStepNow)
{
	return	((HOST_ISOLATED(j) && (Hosts[j].digitalContactTraced != 1)) ? P.CaseIsolationHouseEffectiveness : 1.0)
		...
		*   P.infectiousness[TimeStepNow - Hosts[j].latent_time - 1];
}

double CalcPlaceInf(int j, int k, unsigned short int TimeStepNow)
{
	return	((HOST_ISOLATED(j) && (Hosts[j].digitalContactTraced != 1)) ? P.CaseIsolationEffectiveness : 1.0)
		...
		*	P.PlaceTypeTrans[k] / P.PlaceTypeGroupSizeParam1[k] * CalcPersonInf(j, TimeStepNow);
}

double CalcSpatialInf(int j, unsigned short int TimeStepNow)
{
	return	((HOST_ISOLATED(j) && (Hosts[j].digitalContactTraced != 1)) ? P.CaseIsolationEffectiveness : 1.0)
		*	((Hosts[j].digitalContactTraced==1) ? P.DCTCaseIsolationEffectiveness : 1.0)
		...
		*	CalcPersonInf(j, TimeStepNow); 		/*	*Hosts[j].spatial_norm */
}

double CalcPersonInf(int j, unsigned short int TimeStepNow)
{
...
double CalcPlaceSusc(int ai, int k, unsigned short int TimeStepNow)
{
	return		((HOST_QUARANTINED(ai) && (!Hosts[ai].care_home_resident) && (Hosts[ai].digitalContactTraced != 1)) ? P.HQuarantinePlaceEffect[k] : 1.0)
		* ((Mcells[Hosts[ai].mcell].socdist == TreatStat::Treated) ? ((Hosts[ai].esocdist_comply) ? P.EnhancedSocDistPlaceEffectCurrent[k] : P.SocDistPlaceEffectCurrent[k]) : 1.0)
