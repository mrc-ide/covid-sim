...
int Latent::DoIncub(int ai, unsigned short int ts, int tn, int run)
{
	...
	q = P->ProportionSymptomatic[age]
		...
		* (HOST_VACCED_PTR(ai) ? (1 - P->VaccSympDrop) : 1); // TODO - fix need for _PTR
	...
	else
	{
		...
		a->Severity_Final = (Severity) ChooseFinalDiseaseSeverity(age, tn);
		// TODO -remove case
...
int Latent::ChooseFinalDiseaseSeverity(int AgeGroup, int tn)
{
